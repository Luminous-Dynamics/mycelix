...
export class AccountingAllocationLineageSource {
  ...
  async appendSuccessorLink(input: SettlementAllocationSuccessorLinkRecordInput): Promise<void> {
    ...
    await this.client.$transaction(async tx => {
      // Do not pre-acquire the advisory lock here. INSERT first acquires the
      // table ROW EXCLUSIVE lock, then the database trigger acquires the shared
      // advisory lock. This order matches all writers and cannot deadlock with
      // snapshot capture, which freezes the tables before taking that lock.
