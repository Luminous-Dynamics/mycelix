...
const obligation = createRoyaltyObligationAuthority({
  id: 'obl:1', beneficiaryId: 'creator:alice', amount: money(500n, 'USD'), observedAt: '2026-09-10T00:00:00Z',
  provenance: { usageEvidenceRef: 'usage:epoch:1', rightsResolutionRef: 'rights:resolution:1', economicTermsRef: 'terms:v1' },
});
const eligibility = {
  id: 'elig:1', obligationId: obligation.id, code: SettlementEligibilityCode.Eligible,
  sourceRef: 'eligibility-authority:v1', observedAt: '2026-09-10T00:01:00Z',
} as const;
const epoch: SettlementEpoch = {
  id: 'epoch:1', cutoff: '2026-09-10T00:02:00Z', eligibilityAsOf: '2026-09-10T00:03:00Z', minimumPayout: money(100n, 'USD'),
};
...
describe('persisted accounting evidence verification', () => {
  it('reconstructs and verifies obligation authority from Prisma-style timestamps', () => {
    ...
    const verified = verifyPersistedRoyaltyObligation({ ...persisted, observedAt: new Date(persisted.observedAt) });
  ...
  it('rejects persisted finality that lacks receipt or amount identity', () => {
    ...
    const noCurrency = { ...persisted } as Record<string, unknown>; delete noCurrency.settledCurrency;
  ...
  it('rechecks statement arithmetic and immutable snapshot commitment', () => {
    ...
    expect(() => verifyPersistedStatementSnapshot({ ...persisted, netPayableMinor: '349' })).toThrow(/gross must equal held \+ deductions \+ netPayable/);
    expect(() => verifyPersistedStatementSnapshot({ ...persisted, snapshotRoot: 'f'.repeat(64) })).toThrow(/canonical projection at snapshotRoot/);
  });
});
