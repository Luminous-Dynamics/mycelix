...
describe('settlement allocation lineage', () => {
  it('accepts monotonic residual resolution by new deduction authority', () => {
    ...
    const resolved = resolveSettlementAllocationLineage([deductionResolved, initial], [link]);
    ...
    expect(resolved.headAllocation.obligationSetDischarged).toBe(true);
  ...
  it('is deterministic and independent of caller input order', () => {
    ...
    const left = resolveSettlementAllocationLineage([initial, deductionResolved], [link]);
    const right = resolveSettlementAllocationLineage([deductionResolved, initial], [link]);
  ...
  it('fails closed on missing lineage links instead of choosing the newest timestamp', () => {
    expect(() => resolveSettlementAllocationLineage([initial, deductionResolved], []))
  ...
  it('resolves a single immutable allocation without inventing a successor', () => {
    const resolved = resolveSettlementAllocationLineage([initial], []);
    expect(resolved.depth).toBe(0);
    expect(resolved.headAllocationRoot).toBe(initial.allocationRoot);
  });
});
