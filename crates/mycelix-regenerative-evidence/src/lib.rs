...
fn validate_resolved_binding(
    ...
    let observation = match resolved {
        ResolvedEvidence::Raw(observation) => {
            observation.validate().map_err(|error| SoilEvidenceError::InvalidResolvedEvidence {
                observation_id: binding.observation_id.clone(),
                reason: error.to_string(),
            })?;
            if !matches!(observation.class, EvidenceClass::Reported | EvidenceClass::Observed) {
        ...
        ResolvedEvidence::Lineaged(product) => {
            product.validate().map_err(|error| SoilEvidenceError::InvalidResolvedEvidence {
                observation_id: binding.observation_id.clone(),
                reason: error.to_string(),
            })?;
            &product.observation
...
impl fmt::Display for SoilEvidenceError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingBindings => f.write_str("soil evidence profile requires at least one binding"),
            Self::TooManyBindings { actual, max } => {
                write!(f, "soil evidence profile has {actual} bindings; maximum is {max}")
            }
            ...
            Self::InvalidCompositeSampleCount(count) => {
                write!(f, "composite sample requires at least two constituents; got {count}")
            }
            Self::DuplicateObservationId(id) => {
                write!(f, "PEF observation {id} is bound more than once")
            }
            Self::MissingResolvedEvidence(id) => write!(f, "missing resolved PEF evidence for {id}"),
            Self::InvalidResolvedEvidence { observation_id, reason } => {
                write!(f, "invalid resolved PEF evidence {observation_id}: {reason}")
            }
            Self::ComputedEvidenceRequiresLineage { observation_id, actual } => write!(
                f,
                "computed PEF evidence {observation_id} with class {actual:?} requires LineagedObservation"
            ),
            Self::ObservationIdMismatch { requested, resolved } => write!(
                f,
                "resolved PEF observation id {resolved} does not exactly match requested {requested}"
            ),
            Self::PhenomenonMismatch { observation_id, expected, actual } => write!(
                f,
                "PEF observation {observation_id} phenomenon {actual} does not match expected {expected}"
            ),
            Self::EvidenceClassMismatch { observation_id, expected, actual } => write!(
                f,
...
mod tests {
    ...
    fn duplicate_observation_id_cannot_be_relabeled() {
        ...
        assert!(matches!(result, Err(SoilEvidenceError::DuplicateObservationId(_))));
    }
    ...
    fn observed_and_reported_raw_evidence_are_admitted() {
        for class in [EvidenceClass::Observed, EvidenceClass::Reported] {
            ...
            profile
                .validate_resolved(|requested| (requested == id).then_some(ResolvedEvidence::Raw(&obs)))
                .unwrap();
        }
    }
