use mycelix_data_rights::{
    assess_terms_compatibility, AssessmentState, DatasetSplitRole, DimensionDisposition,
    ExternalPrerequisite, IntendedDatasetUseV1, IntendedUseDimension, RetainedObligation,
    ReviewedTermsEvidenceV1, TermsCurrentness,
};
...
fn qualified_synthetic_corpus_v0_1_is_reproduced_exactly() {
    let raw = include_str!(
        "../../../docs/embodied/fixtures/EMB_DATA_RIGHTS_COMPATIBILITY_V0_1.json"
    );
    ...
    for case in &corpus.cases {
        ...
        assert_eq!(assessment.state(), expected_state(&case.expected_state), "{}", case.id);
        ...
        let observed_obligations: BTreeSet<_> = assessment
            ...
            .collect();
        assert_eq!(observed_obligations, case.expected_obligations, "{}", case.id);
...
fn corpus_order_is_not_semantic_authority() {
    ...
    for case in corpus.cases.iter().filter(|case| {
        !case.reviewed_terms_evidence.per_source.is_empty()
    }) {
...
fn conflict_and_restrictive_composition_remain_distinct() {
    ...
    let (evidence, request) = materialize(case);
    assert_eq!(
        assess_terms_compatibility(&evidence, &request).unwrap().state(),
