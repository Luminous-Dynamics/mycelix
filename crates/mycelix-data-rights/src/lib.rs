    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ConflictingReuseOfEvidenceRef(reference) => {
                write!(f, "evidence reference is reused with conflicting content: {reference}")
            }
        }
    }
}
...
pub fn assess_terms_compatibility(
    ...
    if request.subjects.iter().any(|subject| subject.trim().is_empty()) {
        return Err(EvidenceError::EmptySubject);
    }
    ...
    if currentness_by_subject.values().any(|states| states.len() > 1) {
        return Ok(make_assessment(
    ...
    if evidence
        .iter()
        .any(|row| matches!(row.currentness, TermsCurrentness::Expired | TermsCurrentness::Superseded))
    {
        return Ok(make_assessment(
...
mod tests {
    ...
    fn duplicate_identical_evidence_ref_normalizes() {
        ...
        let result = assess_terms_compatibility(&[row.clone(), row], &training_request("dataset:a:v1"))
            .expect("identical duplicate should normalize");
        assert_eq!(result.state(), AssessmentState::CompatibleWithReviewedTermsProfile);
    ...
    fn conflicting_reuse_of_same_evidence_ref_is_malformed() {
        ...
        let b = training_evidence("dataset:a:v1", "review:a", DimensionDisposition::Incompatible);
        assert_eq!(
            assess_terms_compatibility(&[a, b], &training_request("dataset:a:v1")),
            Err(EvidenceError::ConflictingReuseOfEvidenceRef("review:a".to_string()))
        );
    }
    ...
    fn distinct_review_refs_can_expose_terms_conflict() {
        let a = training_evidence("dataset:a:v1", "review:a", DimensionDisposition::Compatible);
        let b = training_evidence("dataset:a:v1", "review:b", DimensionDisposition::Incompatible);
        let result = assess_terms_compatibility(&[a, b], &training_request("dataset:a:v1")).unwrap();
    ...
    fn compatibility_for_one_dimension_does_not_imply_another() {
        let evidence = ReviewedTermsEvidenceV1 {
            ...
            currentness: TermsCurrentness::Current,
            dimensions: [(IntendedUseDimension::Training, DimensionDisposition::Compatible)]
                .into_iter()
                .collect(),
            obligations: BTreeSet::new(),
        };
    ...
    fn external_privacy_prerequisite_survives_terms_compatibility() {
        let evidence = training_evidence("dataset:a:v1", "review:a", DimensionDisposition::Compatible);
