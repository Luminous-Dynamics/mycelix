...
- **Trait-based transport** so the same `HolochainClient` type works across:
  ...
  - `TauriIpcTransport` — invoke a Tauri backend command over `holochain_client` (feature `tauri`)
...
| `tauri` | no | Tauri IPC transport (`TauriIpcTransport`) |
...
It is used by several Mycelix ecosystem apps:
