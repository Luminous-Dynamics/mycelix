...
///
/// NarInfo is metadata and has no prior exact representation-size commitment in
/// the v1 request. The explicit ceiling prevents an authenticated peer from
/// turning one valid request into unbounded metadata delivery.
...
/// MCFS response over the exact same authenticated Xenia application channel.
#[derive(Debug, Error)]
pub enum XeniaNixResponseReceiveErrorV1 {
    /// The supplied/opened Xenia application channel is not the Content Fabric
    /// reader domain reserved by CF-07C2.
    ...
    UnexpectedPayloadType {
        ...
        expected: u8,
        /// Actual channel/opened payload-domain byte.
        actual: u8,
    },
    ...
    /// The receiver has lost/dropped its authority-bearing Xenia channel and is
    ...
    /// A raw NAR response did not end at the exact NarSize supplied before any
    /// raw representation bytes were accepted.
    ...
    SequenceOverflow,
    /// Same-channel deadline-bound MCFR request send failed.
    #[error(transparent)]
    Send(#[from] AuthenticatedPeerApplicationChannelErrorV1),
    /// Deadline-owned same-channel response receive/open failed. Xenia returns
    /// no recoverable channel on this path.
    #[error(transparent)]
    Receive(#[from] AuthenticatedPeerApplicationReceiveErrorV1),
}
...
/// convenience. A chunk is **not** proof that the response completed: callers
...
impl ReceivedXeniaNixResponseChunkV1 {
    ...
    ///
    /// The returned bytes are still part of an *in-progress* response until a
    /// valid End frame has been observed.
...
///
/// This proves authenticated-channel framing/sequence/length completion only.
/// It does not prove a raw NAR digest, Nix signature, trusted-public-key policy,
/// or install authorization. Raw NAR integrity still belongs to the subsequent
/// digest/CAS admission boundary.
...
/// one exact MCFR request through that channel before one monotonic round-trip
/// deadline.
///
/// The channel remains owned by this receiver and can be recovered only through
/// [`Self::finish`] after a valid End frame. Before every response await the
/// channel is moved out and handed to Xenia's deadline-owned consuming receive
/// primitive. Xenia returns the channel only on success. A timeout, carrier,
/// domain, AEAD, replay, wire, parser, or caller-cancellation failure therefore
/// leaves no recoverable authority-bearing channel.
...
impl<T: Transport> XeniaNixResponseReceiverV1<T> {
    /// Receive and validate the next authenticated MCFS frame before the one
    /// request/response deadline supplied when the MCFR request was sent.
    ///
    /// The receiver takes its channel out before awaiting Xenia. If this future
    /// is cancelled, the moved channel is dropped with Xenia's receive future;
    /// this receiver is left with no channel and cannot later finish/reuse it.
    pub async fn recv_next(
        &mut self,
    ) -> Result<XeniaNixResponseEventV1, XeniaNixResponseReceiveErrorV1> {
        if self.terminal || self.channel.is_none() {
            self.terminal = true;
        ...
        let (channel, opened) = match recv_opened_payload_before_deadline_v1(channel, self.deadline).await {
            Ok(value) => value,
            Err(error) => {
                self.terminal = true;
        ...
        let parsed = match self.state.apply_frame(opened.plaintext()) {
            Ok(parsed) => parsed,
            Err(error) => {
                self.terminal = true;
                return Err(error);
        ...
        // Reinsert the channel only after both Xenia receive/open and MCFS
        // structural validation succeeded. Every error above drops `channel`.
        self.channel = Some(channel);

        match parsed {
            ParsedResponseEventV1::Data {
                bytes,
                sequence,
                cumulative_bytes,
            ...
            ParsedResponseEventV1::End => Ok(XeniaNixResponseEventV1::End),
        }
    }
    ...
    /// is no recovery API for a terminal, cancelled, or incomplete response.
    pub fn finish(
        mut self,
    ) -> Result<
        (
...
/// Send one NarInfo MCFR request before `deadline` and return a receiver that
/// owns the exact same authenticated Xenia channel until the matching MCFS
/// response completes before that same deadline.
...
/// Send one raw-NAR MCFR request before `deadline` and return a receiver that
/// owns the exact same channel until the complete matching MCFS response arrives
/// before that same deadline.
///
...
async fn send_request_v1<T: Transport>(
    ...
    channel.send_payload_before_deadline(&request, deadline).await?;
