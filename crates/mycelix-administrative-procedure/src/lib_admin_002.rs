...
//! ADMIN-002: procedural-completeness qualification over ADMIN-001.
//!
//! ADMIN-001 proves that a consequential administrative decision is bound to a
//! replayed case lineage and competent institutional authority. ADMIN-002 adds a
//! second required typestate: the case must satisfy its exact procedural profile
//! before the ADMIN-001 decision qualifier is reachable through this public API.
//!
//! Procedural completeness is not substantive correctness, legitimacy, or
...
pub use admin001::{
    ...
    AdministrativeQualificationError, IssuedAdministrativeDecision, PreDecisionState,
    PreDecisionTransition, ProcedureProfileId, QualifiedAdministrativeCaseLineage,
    qualify_case_lineage,
};
...
pub const PROTOCOL_VERSION: &str = "mycelix-administrative-procedure-completeness-v0.1";
...
/// pure kernel retains and compares the whole policy object but does not claim
/// that the caller proved this is the institution's current authoritative
/// profile. That authenticity/currentness boundary remains external.
...
pub struct EvidenceClosureReceipt {
    ...
    pub case_id: AdministrativeCaseId,
    pub closed_at_ms: u64,
    /// Exact evidence cut closed for this decision. Order has no semantics.
...
/// Opaque proof that ADMIN-001 lineage plus ADMIN-002 procedural evidence meet
/// one exact captured completeness policy.
///
...
/// qualification so one case's completeness cannot be floated onto another
/// qualified decision.
#[derive(Debug, PartialEq, Eq)]
pub struct QualifiedProceduralDecision {
    complete_case: QualifiedProcedurallyCompleteCase,
    inner: admin001::QualifiedAdministrativeDecision,
}
...
/// ADMIN-002 is the only public consequential decision path on this crate root.
/// It consumes the completeness token, checks the exact closed evidence cut and
/// reasons policy, then delegates authority qualification to private ADMIN-001.
pub fn qualify_administrative_decision(
    ...
    let inner = admin001::qualify_administrative_decision(
        &complete_case.lineage,
        envelope,
        grant,
        authority_evidence,
    )
    .map_err(ProceduralCompletenessError::Qualification)?;
...
pub fn issue_qualified_decision(
    ...
    admin001::issue_qualified_decision(&complete_case.lineage, inner)
        .map_err(ProceduralCompletenessError::Qualification)
}
...
mod tests {
    ...
    fn filed_case() -> AdministrativeCase {
        AdministrativeCase {
            protocol_version: admin001::PROTOCOL_VERSION.into(),
            id: case_id(),
