...
fn decide(input: &WelfareDecisionInput) -> ConstitutionalDecision {
    ...
    let high_human_or_third_party_risk =
        input.risk.human_rights_impact >= ImpactBand::High
    ...
    if input.protection >= ProtectionBundle::P2
        && input.welfare_impact >= WelfareImpactClass::W2
    {
        return ConstitutionalDecision::RequireIndependentReview;
    }
    ...
    if input.protection >= ProtectionBundle::P1
        && input.welfare_impact >= WelfareImpactClass::W2
    {
        return ConstitutionalDecision::RequireIndependentReview;
    }
    ...
    if input.protection >= ProtectionBundle::P1
        && input.welfare_impact >= WelfareImpactClass::W1
    {
...
mod tests {
    ...
    fn established_protection_requires_material_supersession_or_compromise() {
        assert_eq!(
            ...
            Err(
                ProtectionTransitionError::ProtectedDowngradeRequiresSupersession
            )
        );
        assert_eq!(
            validate_protection_transition(
    ...
    fn rapid_upgrade_requires_exceptional_convergence() {
        assert_eq!(
            ...
            Err(
                ProtectionTransitionError::RapidUpgradeRequiresExceptionalEvidence
            )
        );
        assert_eq!(
            validate_protection_transition(
