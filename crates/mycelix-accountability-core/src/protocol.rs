...
//! Semantic reciprocal-accountability protocol types and fail-closed validation.
//!
//! This module intentionally contains no cryptographic serialization logic.
//! `canonical` owns the language-neutral commitment profile; this module owns
//! the human/privacy semantics that must be true before and after evidence is
//! attached.
...
pub struct AccessReceipt {
    ...
    /// Proof references attached after the pre-attestation receipt commitment is
...
pub fn project_subject_notice(
    ...
    let inference = if policy.include_inference_summary {
        receipt
            .inference
            .as_ref()
            .map(|value| SubjectInferenceNotice {
...
/// Phase-one validation. This validates semantic/privacy invariants and the
...
pub fn validate_receipt(
    ...
    for required in &policy.required_attestation_roles {
        if !roles.contains(required) {
            return Err(AccountabilityError::MissingRequiredAttestationRole(
                *required,
            ));
        }
    }
...
fn validate_delay(
    ...
    for approval in &auth.approvals {
        ...
        let identity = (
            approval.organization_id.clone(),
...
mod tests {
    ...
    fn final_validation_requires_all_configured_evidence_roles() {
        ...
        receipt
            .attestations
            .push(proof(AttestationRole::ExecutionBinding, 40));
        assert_eq!(
            validate_receipt(&receipt, &policy()),
            Err(AccountabilityError::MissingRequiredAttestationRole(
                AttestationRole::ComputationProof
        ...
        receipt
            .attestations
            .push(proof(AttestationRole::ComputationProof, 50));
    ...
    fn required_subject_rights_are_fail_closed() {
        ...
        receipt
            .rights
            .retain(|right| *right != SubjectRight::Contest);
        assert_eq!(
            validate_receipt(&receipt, &policy()),
