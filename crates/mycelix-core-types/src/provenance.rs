...
///
/// Fields are private so callers cannot bypass the non-empty namespace/id
/// invariant with a struct literal.
...
///
/// Fields are private: known algorithms cannot be paired with malformed output
/// lengths by bypassing `ContentDigest::new`.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
...
///
/// Independence is intentionally absent from this enum. It requires a separate
/// evidence-bearing `IndependenceAssessment` and cannot be asserted as a bare
/// relationship label.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
...
impl IndependenceAssessment {
    pub fn new(
        ...
        if status == IndependenceStatus::Independent && left_family == right_family {
            return Err("the same provenance family cannot be independently corroborating itself");
        }
