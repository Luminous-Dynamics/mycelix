    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ExcludedModalityNotInSource(modality) => {
                write!(f, "excluded modality is not present in source: {modality:?}")
            }
...
fn validate_request(
    ...
    if let Some(modality) = request.requested_modalities.difference(&available).next() {
        return Err(PrivacyEvidenceError::RequestedModalityUnavailable(*modality));
    }
    Ok(())
}
...
mod tests {
    ...
    fn same_subject_distinct_reviews_can_form_evidence_conflict() {
        ...
        assert_eq!(result.state(), PrivacyPurposeAssessmentState::EvidenceConflict);
    }
    ...
    fn reviewed_bystander_transform_must_cover_requested_representation() {
        ...
        assert_eq!(
            ...
            Err(PrivacyEvidenceError::RequestedModalityOutsideReviewedTransformation(
                Modality::HandPose
            ))
        );
    }
