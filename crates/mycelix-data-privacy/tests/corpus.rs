...
fn purpose_map(value: &Value) -> BTreeMap<PrivacyPurpose, PurposeDisposition> {
    value
        ...
        .map(|(purpose, disposition)| {
            (
                serde_json::from_value(Value::String(purpose.clone())).expect("known purpose"),
                parse(disposition),
            )
        })
        .collect()
...
fn evidence_for_case(case: &Value) -> Vec<ReviewedPrivacyPurposeEvidenceV1> {
    ...
    vec![ReviewedPrivacyPurposeEvidenceV1 {
        ...
        evidence_ref: format!("fixture:{}:root", case["id"].as_str().expect("case id")),
        currentness: root_currentness,
        participant_evidence,
        bystander_evidence,
...
fn exact_qualified_c01_c20_corpus_replays() {
    ...
    assert_eq!(
        document["authority"],
        "SyntheticPrivacyPurposeSemanticsOnly"
    );
    for field in [
        ...
        assert_eq!(
            ...
            "authority flag {field} must remain false"
        );
    }
...
fn terms_reference_has_no_decision_path_and_data_minimization_is_preserved() {
    ...
    let c19 = cases
        ...
        .expect("C19");
    assert!(
        c19["reviewed_privacy_purpose_evidence"]
            .get("terms_compatibility_ref")
            .is_some()
    );
    let result =
        assess_privacy_purpose_compatibility(&evidence_for_case(c19), &request_for_case(c19))
    ...
    let c03 = cases
        ...
        .expect("C03");
    let c03_result =
        assess_privacy_purpose_compatibility(&evidence_for_case(c03), &request_for_case(c03))
    ...
    let c11 = cases
        ...
        .expect("C11");
    let c11_result =
        assess_privacy_purpose_compatibility(&evidence_for_case(c11), &request_for_case(c11))
            .expect("C11 should assess");
    assert!(
        c11_result
            .retained_nonclaims()
            .iter()
            .any(|row| { row.nonclaim() == PrivacyNonclaim::ReidentificationImpossible })
    );

    let c20 = cases
        .iter()
        .find(|case| {
            case["id"] == "C20-compatible-never-creates-publication-or-actuation-authority"
        })
        .expect("C20");
    let c20_result =
        assess_privacy_purpose_compatibility(&evidence_for_case(c20), &request_for_case(c20))
