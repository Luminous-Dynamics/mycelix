...
//! FORGE-004D4B: acyclic concrete host orchestration for one M0 Forge run.
//!
//! This is the v2 host receipt. The process/NAR/isolation lifecycle is inherited
//! from D3B2D2C, but input transport is the acyclic 5+3 sealed capsule theorem
//! from D4A instead of treating control-plane files as policy ancestors.
...
use mycelix_forge_guest_output_frame::{read_guest_envelope_frame, GuestOutputFrameError};
...
use mycelix_forge_nar_auditor::{audit_and_qualify_runtime_closure, NarAuditorError};
use mycelix_forge_runtime_closure_evidence::QualifiedRuntimeClosureEvidence;
use mycelix_forge_sealed_capsule_inputs::{
    build_m0_capsule_bubblewrap_command, seal_m0_capsule_inputs,
    QualifiedSealedM0CapsuleEvidence, SealedCapsuleError,
};
use semver::Version;
...
const HOST_EVIDENCE_DOMAIN_V2: &[u8] = b"mycelix-forge/hermetic-host-evidence/v2\0";
...
pub struct HermeticHostInputsV2<'a> {
...
pub struct QualifiedHermeticHostRunV2 {
    ...
    sealed_capsule: QualifiedSealedM0CapsuleEvidence,
...
impl QualifiedHermeticHostRunV2 {
    ...
    pub fn sealed_capsule(&self) -> &QualifiedSealedM0CapsuleEvidence {
        &self.sealed_capsule
    }
...
pub fn execute_hermetic_guest_v2(
    inputs: HermeticHostInputsV2<'_>,
) -> Result<QualifiedHermeticHostRunV2, HermeticHostV2Error> {
    inputs
        .isolation_policy
    ...
    let mut sealed = seal_m0_capsule_inputs(
        inputs.execution_spec,
        inputs.isolation_policy,
        inputs.runtime_artifacts,
    )?;
    let sealed_capsule = sealed.qualified().clone();
    let command = build_m0_capsule_bubblewrap_command(
        inputs.bubblewrap_program,
        inputs.execution_spec,
        inputs.isolation_policy,
        inputs.closure,
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or(HermeticHostV2Error::MissingChildStdout)?;
    let stderr = child
        .stderr
        .take()
        .ok_or(HermeticHostV2Error::MissingChildStderr)?;
    ...
    if pidfd_ready(&pidfd, 0)? {
        return Err(HermeticHostV2Error::ChildExitedWhileBlocked(child_pid));
    }
    ...
    let frame_result = stdout_thread
        .join()
        .map_err(|_| HermeticHostV2Error::ReaderThreadPanicked)?;
    let stderr_bytes = stderr_thread
        .join()
        .map_err(|_| HermeticHostV2Error::ReaderThreadPanicked)??;

    if !bubblewrap_status.success() {
        return Err(HermeticHostV2Error::BubblewrapFailed {
            status: bubblewrap_status.code().unwrap_or(-1),
            stderr: String::from_utf8_lossy(&stderr_bytes).into_owned(),
        });
    }
    if !pidfd_ready(&pidfd, PIDFD_EXIT_TIMEOUT_MS)? {
        return Err(HermeticHostV2Error::PidfdDidNotObserveExit(child_pid));
    }
    ...
    if guest.tool_map_digest() != guest_tools.map_digest() {
        return Err(HermeticHostV2Error::GuestToolMapQualificationMismatch);
    }
    ...
    if isolation.inside_digest() != guest.inside_digest() {
        return Err(HermeticHostV2Error::InsideIsolationMismatch);
    }
    ...
    if pre_runtime.evidence_digest() != post_runtime.evidence_digest() {
        return Err(HermeticHostV2Error::RuntimeClosureChangedDuringRun);
    }
    ...
    let evidence_digest = derive_host_evidence_v2(
        &execution_spec_digest,
        &run_plan_digest,
        &host_executable,
        ...
        sealed_capsule.evidence_digest(),
...
) -> Result<(), HermeticHostV2Error> {
    if run_plan.execution_spec() != execution_spec {
        return Err(HermeticHostV2Error::RunPlanExecutionSpecMismatch);
    }
    if run_plan.isolation_policy() != isolation_policy {
        return Err(HermeticHostV2Error::RunPlanIsolationPolicyMismatch);
    }
    if run_plan.git_object_validation_policy() != guest_plan.git_object_validation_policy() {
        return Err(HermeticHostV2Error::RunPlanGitPolicyMismatch);
    }
    if run_plan.run_challenge() != guest_plan.run_challenge() {
        return Err(HermeticHostV2Error::RunChallengeMismatch);
    }
    Ok(())
}
...
fn require_host_executables(
...
) -> Result<(String, String), HermeticHostV2Error> {
    let host = std::env::current_exe()?;
    let host = host
        .to_str()
        .ok_or(HermeticHostV2Error::NonUtf8ExecutablePath)?
        .to_owned();
    ...
    let host_tool = spec
        .tools()
        .iter()
        .find(|tool| tool.role() == "forge-hermetic-host")
        .ok_or_else(|| HermeticHostV2Error::MissingExecutionTool("forge-hermetic-host".into()))?;
    if host_tool.semantic_version() != "0.2.0" {
        return Err(HermeticHostV2Error::HostVersionMismatch(
            host_tool.semantic_version().to_owned(),
        ));
    }
    ...
    let bubblewrap_tool = spec
        ...
        .ok_or_else(|| HermeticHostV2Error::MissingExecutionTool("bubblewrap".into()))?;
    ...
    if version < Version::parse(BUBBLEWRAP_MIN_VERSION).expect("fixed version is valid") {
        return Err(HermeticHostV2Error::BubblewrapVersionTooOld(
...
fn validate_exact_store_executable(
    ...
    if !executable.starts_with("/nix/store/")
        ...
        || executable.contains("/./")
    {
        return Err(HermeticHostV2Error::InvalidHostExecutable(
            executable.to_owned(),
        ));
    }
    let tail = executable
        .strip_prefix("/nix/store/")
        .ok_or_else(|| HermeticHostV2Error::InvalidHostExecutable(executable.to_owned()))?;
    let (entry, _) = tail
        .split_once('/')
        .ok_or_else(|| HermeticHostV2Error::InvalidHostExecutable(executable.to_owned()))?;
    ...
    {
        return Err(HermeticHostV2Error::HostExecutableOutsideClosure(
            executable.to_owned(),
        ));
    }
    Ok(())
...
fn spawn_bubblewrap(command: BubblewrapCommand) -> Result<Child, HermeticHostV2Error> {
...
fn pipe_cloexec() -> Result<(OwnedFd, OwnedFd), HermeticHostV2Error> {
    let mut fds = [0_i32; 2];
...
fn clear_cloexec(fd: RawFd) -> Result<(), HermeticHostV2Error> {
...
) -> Result<PendingParentObservation, HermeticHostV2Error> {
    let mut status = Vec::new();
    loop {
        let mut line = Vec::new();
        let read = reader.read_until(b'\n', &mut line)?;
        if read == 0 {
            return Err(HermeticHostV2Error::MissingChildStartStatus);
        }
        status.extend_from_slice(&line);
        if status.len() > MAX_STATUS_BYTES {
            return Err(HermeticHostV2Error::StatusStreamTooLarge);
        }
...
fn pidfd_open(pid: u32) -> Result<OwnedFd, HermeticHostV2Error> {
    let pid = i32::try_from(pid).map_err(|_| HermeticHostV2Error::InvalidChildPid)?;
...
fn pidfd_ready(pidfd: &OwnedFd, timeout_ms: i32) -> Result<bool, HermeticHostV2Error> {
    ...
    if entry.revents & libc::POLLNVAL != 0 {
        return Err(HermeticHostV2Error::InvalidPidfd);
    }
    Ok(result > 0
...
fn derive_host_evidence_v2(
    ...
    post_runtime: &Digest,
    same_run: &Digest,
) -> Result<Digest, HermeticHostV2Error> {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(HOST_EVIDENCE_DOMAIN_V2);
...
fn push_string(out: &mut Vec<u8>, value: &str) -> Result<(), HermeticHostV2Error> {
    let len =
        u16::try_from(value.len()).map_err(|_| HermeticHostV2Error::CanonicalLengthOverflow)?;
    out.extend_from_slice(&len.to_be_bytes());
    out.extend_from_slice(value.as_bytes());
    Ok(())
}

fn push_digest(out: &mut Vec<u8>, digest: &Digest) -> Result<(), HermeticHostV2Error> {
    push_string(out, digest.algorithm().id())?;
    let len = u16::try_from(digest.as_bytes().len())
        .map_err(|_| HermeticHostV2Error::CanonicalLengthOverflow)?;
    out.extend_from_slice(&len.to_be_bytes());
...
#[derive(Clone, Debug, Deserialize)]
pub struct HostLaunchManifestV2 {
...
pub fn execute_from_launch_manifest_v2(
    manifest_path: &Path,
) -> Result<QualifiedHermeticHostRunV2, HermeticHostV2Error> {
    let manifest: HostLaunchManifestV2 = load_json(manifest_path)?;
    ...
    execute_hermetic_guest_v2(HermeticHostInputsV2 {
...
fn load_json<T: serde::de::DeserializeOwned>(path: &Path) -> Result<T, HermeticHostV2Error> {
    let bytes = std::fs::read(path)?;
    Ok(serde_json::from_slice(&bytes)?)
}

#[derive(Debug, Error)]
pub enum HermeticHostV2Error {
    #[error(transparent)]
    ...
    #[error(transparent)]
    SealedCapsule(#[from] SealedCapsuleError),
    #[error(transparent)]
    ...
    #[error("execution spec names a non-v2 hermetic host version: {0}")]
