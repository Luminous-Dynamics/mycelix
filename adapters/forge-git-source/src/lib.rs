...
//! FORGE-009C: concrete pinned-Git source verifier for FORGE-009B.
//!
//! This ports the useful FORGE-008B Git plumbing semantics onto the current
//! protected-merge source interface without importing obsolete collaboration
//! ancestry. The adapter proves Git object/type/tree/ancestry facts only.
//!
//! It does not establish repository closure, M0 OfflineEvidence, repository-tip
//! currentness, merge authorization, or merge execution.
...
use mycelix_forge_protected_merge_source::{
    ProposalSourceObservationV1, ProposalSourceVerifierV1, SourceVerifierErrorV1,
    SourceVerifierIdentityV1,
};
...
/// Minimal Git process abstraction used by the concrete verifier and tests.
...
/// Process-backed Git runner with an exact absolute executable path and cleared
...
impl ProcessGitRunner {
    /// Construct a process runner. Relative executable paths are rejected so
    /// the adapter never searches ambient `PATH`.
    ...
    /// Exact configured Git executable path.
...
/// Concrete pinned Git source verifier for the current FORGE-009B interface.
#[derive(Clone, Debug)]
pub struct GitProposalSourceAdapter<R> {
    runner: R,
    repository: PathBuf,
    expected_git_version: String,
    identity: SourceVerifierIdentityV1,
}
    ...
    R: GitCommandRunner,
{
    ...
    pub fn new(
        ...
        let identity = SourceVerifierIdentityV1::new("git-plumbing", expected_git_version.clone())
    ...
    /// Exact source verifier identity exported into FORGE-009B evidence.
    pub fn identity(&self) -> &SourceVerifierIdentityV1 {
        &self.identity
    }

    /// Build a raw current-line proposal-source observation by interrogating
    /// Git. The observation is still non-authoritative until FORGE-009B invokes
    /// this adapter again through [`ProposalSourceVerifierV1`].
    pub fn observe(
        &self,
        proposal: &ChangeProposal,
        request: &RepositoryVerificationRequest,
    ) -> Result<ProposalSourceObservationV1, GitSourceAdapterError> {
        ...
        let request_digest = request
            ...
            .map_err(|error| GitSourceAdapterError::Protocol(error.to_string()))?;
        Ok(ProposalSourceObservationV1::new(
            self.identity.clone(),
            proposal_id,
            request_digest,
...
impl<R> ProposalSourceVerifierV1 for GitProposalSourceAdapter<R>
where
    R: GitCommandRunner,
{
    ...
    fn verify_source(
        ...
        let inspected = self.inspect(proposal).map_err(|_| SourceVerifierErrorV1::Rejected)?;
        ...
        {
            return Err(SourceVerifierErrorV1::Rejected);
        }
...
mod tests {
    ...
    use mycelix_forge_repository::{RepositoryPolicyState, RepositoryRef};
    ...
    fn proposal() -> ChangeProposal {
        let project = project();
        let principal = PrincipalId::new(digest(0x20));
        ...
        let repository_policy =
            RepositoryPolicyState::new(project, 0, None, digest(0x23)).unwrap();
    ...
    #[test]
    fn exact_git_semantics_produce_current_line_source_state() {
        let proposal = proposal();
        let adapter = GitProposalSourceAdapter::new(
            FakeRunner::successful(&proposal),
    ...
    fn tree_mismatch_fails_closed() {
        ...
        .unwrap();
        assert_eq!(
            adapter.inspect(&proposal).unwrap_err(),
            GitSourceAdapterError::TreeMismatch
        );
    }
    ...
    fn non_ancestor_fails_closed() {
        ...
        assert_eq!(
            adapter.inspect(&proposal).unwrap_err(),
            GitSourceAdapterError::BaseNotAncestor
        );
    }
