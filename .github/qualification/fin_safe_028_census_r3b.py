...
def classify(path: pathlib.PurePosixPath, line: str, token: str) -> str:
    ...
    # R3b changes only identity projection, not the measured source population.
    p = path.as_posix()
    lower = p.lower()
    stripped = line.strip()
...
def blob_text(blob_sha: str):
    data = run("git", "cat-file", "blob", blob_sha)
    try:
...
def main() -> None:
    ...
    # Cross-version population theorem: R3b may deduplicate identities, but it
    ...
    payload = {
        "schema": "mycelix.finance.fin-safe-028.governance-gate-census.v0.3",
        ...
        "blob_read_rule": "git cat-file blob <sha>; never follow worktree paths",
        "tokens": list(TOKENS),
    ...
    json_path = pathlib.Path("/tmp/fin-safe-028-governance-gate-census-r3b.json")
    json_path.write_bytes(raw)
    digest = hashlib.sha256(raw).hexdigest()
    pathlib.Path("/tmp/fin-safe-028-governance-gate-census-r3b.sha256").write_text(
        f"{digest}  fin-safe-028-governance-gate-census-r3b.json\n"
    )

    lines = [
        "# FIN-SAFE-028 governance gate census R3b",
        "",
    ...
    pathlib.Path("/tmp/fin-safe-028-governance-gate-census-r3b.md").write_text(
        "\n".join(lines) + "\n"
    )
