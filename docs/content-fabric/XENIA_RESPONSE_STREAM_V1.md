...
MCFS response data frames
        ↓
Xenia deadline check before seal
        ↓
AEAD seal
        ↓
Xenia deadline check before carrier I/O
        ↓
deadline-bounded carrier send
        ↓
MCFS End frame
        ↓
...
The bridge pins Xenia PR #300 exact commit:

```text
07b1d57d65b0349327e1e986e6e5869f55b8a1c9
```

which is an additive successor to PR #281's sealed authenticated application channel.
...
The same monotonic deadline is passed into Xenia's `send_payload_before_deadline(...)` API.

Inside the sealed Xenia application channel, that method checks the deadline:

1. before AEAD sealing;
2. immediately after sealing and before any carrier I/O;
3. around the same-peer carrier-send future with `tokio::time::timeout_at`; and
4. immediately after carrier completion.

The outer C5 `timeout_at` remains as defense in depth around the private sink path.

If either layer observes expiry, C5 returns an error and does not return the channel. If expiry occurs after AEAD sealing, Xenia terminalizes the application channel rather than presenting advanced wire-send state as ordinary reusable state.

A send that completes but is observed to have crossed the deadline also causes C5/Xenia to fail and drop the channel.
...
2. C5 does not initiate a new response-frame send after it has observed serving authority expiry;
3. Xenia does not intentionally enter carrier I/O after its **post-seal** deadline check has observed expiry;
4. each carrier-send future is bounded by the same conservative monotonic authority deadline;
5. a timed-out/failed/late send makes the authority-bearing channel unavailable for reuse through this API; and
6. only a complete response ending in a valid `End` frame returns the channel to the caller.

CF-07C5/Xenia do **not** claim that bytes already handed to an OS/kernel/NIC/network buffer before a deadline cannot arrive at the peer afterward. Expiry cannot retroactively revoke bytes already committed below the application layer.

This is an application/carrier admission guarantee, not proof of remote receipt time. Proving remote receipt before a deadline would require authority-aware acknowledgment semantics at the remote endpoint.
...
If cancellation occurs during the nested Xenia carrier-send future, the outer C5 future owns and drops the channel rather than returning it for reuse.

This preserves confidentiality even though Rust/Tokio cannot forcibly abort an already-running `spawn_blocking` closure or recall network bytes already committed below the application layer.
...
- exact Xenia PR #300 dependency-source verification;
...
The inherited CF-07C2/C3/C4 qualification lanes on the C5 head must also resolve the same exact Xenia #300 commit so the cross-repo upgrade is requalified through every downstream boundary.
