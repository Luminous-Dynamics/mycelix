...
Status: executable research contract. Language-neutral, deterministic, zero-network, and local-authority scoped.
...
Only a commitment-valid LEX-NET-028 `LocalCapabilityLease` is accepted. It must bind exact local domain/grant, subject/purpose/resource/action, mint time, expiry, positive use budget, satisfaction-evaluation commitment, and mint-authorization commitment. Tampered or wrong-kind source products are rejected.
