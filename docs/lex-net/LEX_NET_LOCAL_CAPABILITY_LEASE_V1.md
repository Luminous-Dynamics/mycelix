...
Status: executable research contract, R3. Language-neutral, deterministic, zero-network, and local-authority scoped.
...
## Presented-envelope structural validation

A presented envelope is not accepted merely because its commitment recomputes. Before local use, the reference profile independently checks:

- non-empty source-lease commitment;
- non-empty local domain and local grant commitment;
- non-empty subject, purpose, resource, and action;
- presenter-binding type remains one of `key`, `session`, or `principal`;
- non-empty presenter-binding commitment;
- integer `valid_from`, `current_until`, and use budget;
- `valid_from <= current_until`;
- positive use budget;
- non-empty replay domain;
- non-empty revocation-handle commitment;
- non-empty policy-epoch commitment;
- transferability remains `false`;
- redelegation remains `false`;
- delegation depth remains `0`.

`commitment-valid envelope != structurally valid envelope`.

This protects local consumers from accepting a self-consistent hash over a malformed envelope that never passed the constructor.
