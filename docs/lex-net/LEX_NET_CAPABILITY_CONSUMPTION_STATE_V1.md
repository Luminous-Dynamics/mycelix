...
Status: executable research contract, R5. Language-neutral, deterministic, zero-network, backend-neutral, and closed-schema.
...
- R4 `22bf0ae375fa948299016fe0b8b9f90e98e94694`

R5 does not rewrite them.
...
R5 makes the v1 authority surface closed.
...
Future extensions require a new explicitly versioned profile and qualification. R5 does not infer that an unknown field is safe, non-authoritative, or ignorable.

R5 also revalidates the parent-envelope presenter-binding type as exactly one of `key`, `session`, or `principal`, and accepts an assurance-profile commitment only when it is absent (`null`) or a non-empty string.

## Parent-product provenance boundary

A hash-valid lease envelope is not, by itself, proof that it was legitimately issued by the qualified LEX-NET-025 transition.

`hash-valid parent product != qualified parent provenance`

R5 revalidates the parent envelope's frozen structural invariants that are available in-band, but it does not invent cryptographic provenance or runtime issuance authority that the commitment alone cannot prove.
...
The frozen 50-fixture corpus includes all R3 cases plus:
...
- invalid parent presenter-binding type -> rejected;
- invalid parent assurance-profile binding type -> rejected.

These directly prove the closed-schema, parent-envelope parity, and canonical-genesis refinements.
...
This tranche does not establish that a hash-valid lease envelope was legitimately issued by the qualified LEX-NET-025 transition.

This tranche does not establish production security.

A PASS establishes only the frozen synthetic local capability-consumption state, closed-schema/parent-envelope-parity/canonical-genesis, single-assignment registration, history-conservation, and CAS-transition theorem.
