...
Status: executable research contract, R3. Language-neutral, deterministic, zero-network, and backend-neutral.
...
The store keeps the registered lease and committed states and rejects malformed successors, wrong prior-state bindings, skipped versions, budget drift, stale expected commitments, any rewrite of a prior invocation record, non-monotonic consumption indices, or a transition ID that does not deterministically bind the exact new invocation and prior state.

A caller-supplied string named `current` is not authority.

### Single-assignment registration

Genesis registration is single-assignment per exact lease-envelope commitment. Once a lease has an authoritative current-state pointer, `register()` must return `AlreadyRegistered` and leave the pointer and stored lineage unchanged.

This applies both before the first use and after one or more committed uses. Re-registering the deterministic genesis state must never reset spent budget, erase invocation history, or roll the current pointer backward.

The synthetic store is only the reference oracle. It does not establish database transaction atomicity, backend access-control correctness, Holochain/DHT uniqueness, crash durability, distributed consensus, or production availability.
...
Prior invocation records are immutable across successors. A new successor may add exactly one record and may not alter any existing invocation commitment, transition ID, or consumption index.
...
Registration is one-time for an exact lease commitment. A second registration attempt returns `AlreadyRegistered` and cannot change the authoritative current-state commitment.
...
A PASS establishes only the frozen synthetic local capability-consumption state, single-assignment registration, and CAS-transition theorem.
