...
def source_valid(lease):
    if not (isinstance(lease,dict) and lease.get("product_universe")=="authority"
        ...
        return False
    for key in ("local_domain","local_grant_commitment","subject","purpose","resource","action",
                "satisfaction_evaluation_commitment","mint_authorization_commitment"):
        if not isinstance(lease.get(key),str) or not lease[key]:
            return False
    for key in ("mint_time","current_until","support_current_until","use_budget"):
        if isinstance(lease.get(key),bool) or not isinstance(lease.get(key),int):
            return False
    if lease["use_budget"] <= 0:
        return False
    if not (lease["mint_time"] <= lease["current_until"] <= lease["support_current_until"]):
        return False
    return True
...
def envelope_structural_outcome(x):
    ...
    for key in ("source_lease_commitment","local_domain","local_grant_commitment","subject","purpose","resource","action"):
        if not isinstance(x.get(key),str) or not x[key]:
            return "EnvelopeInvalid"
    if x.get("presenter_binding_type") not in {"key","session","principal"} or not x.get("presenter_binding_commitment"):
        return "PresenterBindingRequired"
    for key in ("valid_from","current_until","use_budget","delegation_depth"):
        if isinstance(x.get(key),bool) or not isinstance(x.get(key),int):
            return "EnvelopeInvalid"
    if x["current_until"] < x["valid_from"] or x["use_budget"] <= 0:
        return "EnvelopeAttenuationViolation"
    if not x.get("replay_domain"):
        return "ReplayDomainRequired"
    if not x.get("revocation_handle_commitment"):
        return "RevocationBindingRequired"
    if not x.get("policy_epoch_commitment"):
        return "PolicyEpochRequired"
    if x.get("transferable") is not False:
        return "TransferDenied"
    if x.get("redelegation_allowed") is not False or x.get("delegation_depth") != 0:
        return "RedelegationDenied"
...
def evaluate(x,**kw):
    ...
    c.update(kw)
    structural = envelope_structural_outcome(x)
    if structural != "IssuedCurrent":
        return structural
...
def run(fid):
    ...
    if fid=="source_missing_local_domain": return bind(source(local_domain=""))["outcome"]
    if fid=="source_missing_grant": return bind(source(local_grant_commitment=""))["outcome"]
    if fid=="source_missing_satisfaction_binding": return bind(source(satisfaction_evaluation_commitment=""))["outcome"]
    if fid=="source_missing_mint_binding": return bind(source(mint_authorization_commitment=""))["outcome"]
    if fid=="source_invalid_lifetime": return bind(source(mint_time=161,current_until=160))["outcome"]
    if fid=="source_zero_budget": return bind(source(use_budget=0))["outcome"]
    if fid=="source_support_horizon_short": return bind(source(current_until=160,support_current_until=159))["outcome"]
    if fid=="invalid_presenter_type": return bind(source(),presenter_binding_type="other")["outcome"]
    ...
    if fid=="presented_missing_source_lease_commitment":
        x=env(); x["source_lease_commitment"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_missing_local_domain":
        x=env(); x["local_domain"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_missing_local_grant":
        x=env(); x["local_grant_commitment"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_invalid_presenter_type":
        x=env(); x["presenter_binding_type"]="other"; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_missing_replay_domain":
        x=env(); x["replay_domain"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_missing_revocation_handle":
        x=env(); x["revocation_handle_commitment"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_missing_policy_epoch":
        x=env(); x["policy_epoch_commitment"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_zero_budget":
        x=env(); x["use_budget"]=0; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_invalid_lifetime":
        x=env(); x["valid_from"]=151; x["current_until"]=150; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
    if fid=="presented_empty_scope":
        x=env(); x["resource"]=""; x["commitment"]=commit(ENV,body(x)); return evaluate(x)
...
def self_test():
    ...
    print("LEX-NET-025 R3 self-test PASS")
def semantic():
    m=load(); seen=set()
    for f in m["fixtures"]:
        got=run(f["id"])
        ...
        seen.add(got)
    print(json.dumps({"tranche":"LEX-NET-025","candidate_revision":"R3","fixture_count":len(m["fixtures"]),
...
def scope():
    ...
    print(json.dumps({"tranche":"LEX-NET-025","candidate_revision":"R3","head":h,"parent":p,"commit_count":n,"paths":paths,"scope_result":"PASS"},sort_keys=True))
def main():
