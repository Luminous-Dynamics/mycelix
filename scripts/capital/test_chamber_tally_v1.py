...
class Tests(unittest.TestCase):
    ...
    def test_duplicate_participant_vote_rejected(self):
        ...
        with self.assertRaisesRegexht.TallyError,'more than once'): self.q(c)
    def test_duplicate_ballot_id_rejected(self):
        ...
        with self.assertRaisesRegexht.TallyError,'duplicate ballot id'): self.q(c)
    ...
    def test_cross_chamber_ballot_rejected(self):
        ...
        with self.assertRaisesRegexht.TallyError,'cross-chamber ballot'): self.q(c)
    ...
    def test_same_participant_multiple_chambers_rejected(self):
        ...
        with self.assertRaisesRegexht.TallyError,'multiple chambers'): self.q(c)
    def test_duplicate_member_in_chamber_rejected(self):
        c=self.case(); c['eligibility_snapshot']['chambers']['USERS'].append('member:u1')
        with self.assertRaisesRegexht.TallyError,'duplicate participant'): self.q(c)
    def test_conflict_wrong_chamber_rejected(self):
        c=self.case(); c['conflicts'][0]['chamber']='PUBLIC'
        with self.assertRaisesRegexht.TallyError,'conflict chamber substitution'): self.q(c)
    ...
    def test_unknown_vote_rejected(self):
        ...
        with self.assertRaisesRegexht.TallyError,'invalid vote value'): self.q(c)
    def test_profile_substitution_rejected(self):
        c=self.case(); c['tally_profile']['governance_profile_sha256']='0'*64
        with self.assertRaisesRegexht.TallyError,'governance profile substitution'): self.q(c)
    ...
    def test_snapshot_wrong_project_rejected(self):
        ...
        with self.assertRaisesRegexht.TallyError,'snapshot project substitution'): self.q(c)
