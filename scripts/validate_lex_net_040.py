...
LEASE_KEYS={"product_universe","kind","source_lease_commitment","local_domain","local_grant_commitment","subject","purpose","resource","action","presenter_binding_type","presenter_binding_commitment","valid_from","current_until","use_budget","replay_domain","revocation_handle_commitment","policy_epoch_commitment","assurance_profile_commitment","transferable","redelegation_allowed","delegation_depth","commitment"}
STATE_KEYS={"product_universe","kind","local_domain","lease_envelope_commitment","version","prior_state_commitment","lease_use_budget","uses_consumed","current_until","replay_domain","revocation_handle_commitment","policy_epoch_commitment","invocation_records","commitment"}
INVOCATION_KEYS={"kind","invocation_id","subject","purpose","resource","action","replay_domain","payload_commitment","commitment"}
RECORD_KEYS={"invocation_commitment","transition_id","consumption_index"}
...
def exact(x,keys): return isinstance(x,dict) and set(x)==keys
...
def lease_ok(x):
    if not (exact(x,LEASE_KEYS) and x.get("product_universe")=="authority" and x.get("kind")=="LocalCapabilityLeaseEnvelope" and ok(x,LD)): return False
    ...
    if x.get("presenter_binding_type") not in {"key","session","principal"}: return False
    ap=x.get("assurance_profile_commitment")
    if ap is not None and (not isinstance(ap,str) or not ap): return False
...
def inv_ok(x):
    if not (exact(x,INVOCATION_KEYS) and x.get("kind")=="CapabilityInvocation" and ok(x,ID)): return False
...
def state_status(s,l=None):
    if not (exact(s,STATE_KEYS) and s.get("product_universe")=="authority" and s.get("kind")=="CapabilityConsumptionState" and ok(s,SD)): return "StateInvalid"
    ...
    for iid,r in rs.items():
        if not isinstance(iid,str) or not iid or len(iid.encode())>MAX_INVOCATION_ID_BYTES or not exact(r,RECORD_KEYS): return "StateInvalid"
...
class AuthorityStateStore:
    ...
    def register(self,l,s):
        ...
        g=genesis(l)
        if g.get("outcome")!="TransitionCommitted" or s!=g["state"]: return "StateInvalid"
...
def run(fid):
    ...
    if fid=="lease_extra_field_rejected":
        x=lease(); x["unknown_authority"]="x"; x["commitment"]=h(LD,b(x)); return "LeaseMismatch" if not lease_ok(x) else "TransitionCommitted"
    if fid=="invocation_extra_field_rejected":
        l,g,s=setup(); x=inv(); x["unknown_authority"]="x"; x["commitment"]=h(ID,b(x)); return consume(l,g,s,x)["outcome"]
    if fid=="state_extra_field_rejected":
        l,g,s=setup(); x=cp(g); x["unknown_authority"]="x"; x["commitment"]=h(SD,b(x)); return state_status(x,l)
    if fid=="record_extra_field_rejected":
        l,g,s,i,r=first(2); x=cp(r["state"]); rec=next(iter(x["invocation_records"].values())); rec["unknown_authority"]="x"; x["commitment"]=h(SD,b(x)); return state_status(x,l)
    if fid=="alternate_genesis_extra_field_rejected":
        l=lease(); g=genesis(l)["state"]; g["unknown_authority"]="x"; g["commitment"]=h(SD,b(g)); s=AuthorityStateStore(); return s.register(l,g)
    if fid=="successor_extra_field_rejected":
        l,g,s=setup(2); ni=inv("inv:2","p2"); x=cp(g); x["version"]=1; x["uses_consumed"]=1; x["prior_state_commitment"]=g["commitment"]; tid=transition_id(l,ni,g["commitment"],1,1); x["invocation_records"]={ni["invocation_id"]:{"invocation_commitment":ni["commitment"],"transition_id":tid,"consumption_index":1}}; x["unknown_authority"]="x"; x["commitment"]=h(SD,b(x)); return s.compare_and_swap(l["commitment"],g["commitment"],x)
    if fid=="lease_invalid_presenter_type_rejected":
        x=lease(presenter_binding_type="other"); return "LeaseMismatch" if not lease_ok(x) else "TransitionCommitted"
    if fid=="lease_invalid_assurance_binding_rejected":
        x=lease(assurance_profile_commitment={"unexpected":"object"}); return "LeaseMismatch" if not lease_ok(x) else "TransitionCommitted"
    raise KeyError(fid)

def load(): return json.loads(M.read_text())
def self_test():
    m=load(); assert len(m["fixtures"])==50; assert m.get("closed_schema") is True; assert h(LD,{"x":1})!=h(SD,{"x":1})!=h(ED,{"x":1}); print("LEX-NET-040 R5 self-test PASS")
def semantic():
    m=load(); seen=set()
    for f in m["fixtures"]:
        got=run(f["id"])
        ...
        seen.add(got)
    print(json.dumps({"tranche":"LEX-NET-040","candidate_revision":"R5","fixture_count":len(m["fixtures"]),"observed_outcomes":sorted(seen),"closed_schema":True,"backend_atomicity_claim":False,"backend_access_control_claim":False,"external_effect_claim":False,"semantic_result":"PASS"},sort_keys=True))
def scope():
    ...
    print(json.dumps({"tranche":"LEX-NET-040","candidate_revision":"R5","head":hd,"parent":p,"commit_count":n,"paths":paths,"scope_result":"PASS"},sort_keys=True))
def main():
