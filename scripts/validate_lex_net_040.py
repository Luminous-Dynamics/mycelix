...
def self_test():
    m=load(); assert len(m["fixtures"])==48; assert m.get("closed_schema") is True; assert h(LD,{"x":1})!=h(SD,{"x":1})!=h(ED,{"x":1}); print("LEX-NET-040 R4 self-test PASS")
def semantic():
    m=load(); seen=set()
    for f in m["fixtures"]:
        got=run(f["id"])
        ...
        seen.add(got)
    print(json.dumps({"tranche":"LEX-NET-040","candidate_revision":"R4","fixture_count":len(m["fixtures"]),"observed_outcomes":sorted(seen),"closed_schema":True,"backend_atomicity_claim":False,"backend_access_control_claim":False,"external_effect_claim":False,"semantic_result":"PASS"},sort_keys=True))
def scope():
    ...
    print(json.dumps({"tranche":"LEX-NET-040","candidate_revision":"R4","head":hd,"parent":p,"commit_count":n,"paths":paths,"scope_result":"PASS"},sort_keys=True))
def main():
