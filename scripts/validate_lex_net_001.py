...
def normalize_prose(text: str) -> str:
    """Normalize presentation-only Markdown for semantic prose checks.

    Machine-governed identifiers, manifests, fixture values, dispositions,
    reasons, and invariant headings are checked separately and remain exact.
    This normalization only prevents harmless inline-code backticks or
    whitespace/capitalization from masquerading as normative drift.
    """
    without_inline_code_delimiters = text.replace("`", "")
    return " ".join(without_inline_code_delimiters.casefold().split())
...
def main() -> None:
    ...
    for claim in nonclaims:
        if normalize_prose(claim) not in doc_prose:
            fail(f"mandatory nonclaim absent from profile: {claim}")

    for phrase in REQUIRED_DOC_PHRASES:
        if normalize_prose(phrase) not in doc_prose:
    ...
    for phrase in forbidden:
        if normalize_prose(phrase) in doc_prose:
