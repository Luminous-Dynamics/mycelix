...
RUSTC_COMMIT = "ac68faa20c58cbccd01ee7208bf3b6e93a7d7f96"
RUSTC_COMMIT_DATE = "2026-05-25"
CARGO_RELEASE = "1.96.0"
CARGO_COMMIT = "30a34c6821b57de0aaec83a901aca39f88f6778c"
CARGO_COMMIT_DATE = "2026-05-25"
LOCK_SCHEMA = "mycelix.psi.002aq.lock.v0.5"
RECEIPT_SCHEMA = "mycelix.psi.002aq.receipt.v0.5"
...
def reject_build_env_overrides(env: dict[str, str] | None = None) -> None:
    source = os.environ if env is None else env
    bad = sorted(
        key
        for key, value in source.items()
        if value
        and (
            key in FORBIDDEN_BUILD_ENV
            or any(key.startswith(prefix) for prefix in FORBIDDEN_BUILD_ENV_PREFIXES)
        )
    )
    if bad:
        raise QualificationError(f"forbidden Rust/Cargo build override(s): {bad}")
...
def clean_env() -> dict[str, str]:
    env = {
        ...
        and key not in FORBIDDEN_BUILD_ENV
        and not any(key.startswith(prefix) for prefix in FORBIDDEN_BUILD_ENV_PREFIXES)
    }
    env.update(
...
def cargo_home(env: dict[str, str] | None = None) -> pathlib.Path:
    source = os.environ if env is None else env
    configured = source.get("CARGO_HOME")
    if configured:
        return pathlib.Path(configured).expanduser().resolve()
    return (pathlib.Path.home() / ".cargo").resolve()


def cargo_config_candidates(cwd: pathlib.Path, env: dict[str, str] | None = None) -> tuple[pathlib.Path, ...]:
    candidates: list[pathlib.Path] = []
    current = cwd.resolve()
    for directory in (current, *current.parents):
        candidates.extend((directory / ".cargo" / "config.toml", directory / ".cargo" / "config"))
    home = cargo_home(env)
    candidates.extend((home / "config.toml", home / "config"))
    return tuple(dict.fromkeys(path.resolve() for path in candidates))


def reject_cargo_config(cwd: pathlib.Path, env: dict[str, str] | None = None) -> None:
    present = [str(path) for path in cargo_config_candidates(cwd, env) if path.exists()]
    if present:
        raise QualificationError(f"Cargo config file(s) present in qualification search path: {present}")
    ...
    return {
        ...
        "qualifier": {
            ...
            "execution": {
                ...
                "rust_cargo_build_overrides_rejected": True,
                "cargo_config_search_path_must_be_empty": True,
            },
        },
        "toolchain": {
            ...
            "cargo_commit_date": CARGO_COMMIT_DATE,
        },
...
def parse_verbose_field(text: str, field: str) -> str | None:
...
def verify_toolchain_versions(rustc: dict[str, Any], cargo: dict[str, Any]) -> dict[str, str]:
    ...
    observed = {
        "rustc_release": parse_verbose_field(rustc["stdout"], "release"),
        "rustc_commit_hash": parse_verbose_field(rustc["stdout"], "commit-hash"),
        "rustc_commit_date": parse_verbose_field(rustc["stdout"], "commit-date"),
        "cargo_release": parse_verbose_field(cargo["stdout"], "release"),
        "cargo_commit_hash": parse_verbose_field(cargo["stdout"], "commit-hash"),
        "cargo_commit_date": parse_verbose_field(cargo["stdout"], "commit-date"),
    }
    expected = {
        "rustc_release": RUSTC_RELEASE,
        "rustc_commit_hash": RUSTC_COMMIT,
    ...
    for key, value in expected.items():
        if observed.get(key) != value:
            raise QualificationError(f"toolchain identity mismatch for {key}: expected {value}, observed {observed.get(key)}")
...
def runtime_versions(workspace: pathlib.Path) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    cargo = run_cmd(["cargo", "-Vv"], workspace)
...
def qualify(repo: pathlib.Path, receipt_output: pathlib.Path) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        ...
        "proposition": (
            ...
            "official Rust 1.96.0/Cargo 1.96.0 source-identity-bound offline qualification profile."
        ),
        "nonclaims": [
