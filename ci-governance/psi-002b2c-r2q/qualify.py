...
PRODUCT_COMMIT = "547eaf6e7ab55431647f510db3cec905a871c83e"
PRODUCT_TREE = "a3a6163c270cf4291b166eabbfae6d58d0755a5f"
PRODUCT_PARENT = "d63eb7c2dec46c3b0b6faeee3062bdfded0ba3b6"
PRODUCT_PARENT_TREE = "16ad4fe0139d261edaebbcaf25fdafba583c3853"
PREFIX = "ci-governance/psi-002b2c-r2q"
QUALIFIER_PATHS = (
    f"{PREFIX}/README.md",
    f"{PREFIX}/lock.json",
    f"{PREFIX}/qualify.py",
...
PRODUCT_BLOBS = {
    ...
    "mycelix-workspace/mycelix-core/libs/psi-registry-currentness-core/README.md": "68e057b23cc4d180c460b648807f136ab3a1fec6",
    "mycelix-workspace/mycelix-core/libs/psi-registry-currentness-core/src/lib.rs": "762255556d7749dbc36d15d314651abd37fe7461",
}
DEPENDENCY_BLOBS = {
...
LOCK_SCHEMA = "mycelix.psi.002b2c.r2q.lock.v0.1"
RECEIPT_SCHEMA = "mycelix.psi.002b2c.r2q.receipt.v0.1"
COMMANDS = (
    ("cargo", "fmt", "--check", "--all"),
...
def verify_checkout(repo: pathlib.Path) -> tuple[str, str]:
    ...
    if git(repo, "rev-parse", "HEAD^") != PRODUCT_COMMIT:
        raise QualificationError("qualifier is not direct child of B2C r2")
...
def static_probes(repo: pathlib.Path) -> dict[str, Any]:
    ...
    for token in required:
        if token not in source:
            raise QualificationError(f"required r2 currentness token absent: {token}")

    if 'format!("{:?}", self.evidence_class)' in source:
        raise QualificationError("evidence class still encoded through Debug formatting")
    if "BTreeSet<(String, String)>" in source:
        raise QualificationError("provider independence still appears keyed by provider+verifier pair")
    ...
    return {"probe_profile": "psi-002b2c-r2q-v0.1", "committed_test_count": test_count}
...
def qualify(repo: pathlib.Path, receipt_path: pathlib.Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="psi-002b2c-r2q-") as temp:
...
def main() -> int:
    ...
    try:
        ...
        print(f"PSI-002B2C-R2Q FAIL: {exc}")
        return 1
