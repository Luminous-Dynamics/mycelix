...
PRODUCT_COMMIT = "1eca62e9732372cbae1230d957123c57c0f8a49d"
PRODUCT_TREE = "1cbd042eb07a87d0857505096afd8dbecc887b9e"
PRODUCT_PARENT = "10eab4ec3c5833d9230fc9bfde89547a0cea1246"
PRODUCT_PARENT_TREE = "8aa6fa930e17761dd8d23e23160a78342e5744ca"
PREFIX = "ci-governance/psi-002b3b2aq"
PATHS = (
    f"{PREFIX}/README.md",
    f"{PREFIX}/lock.json",
    f"{PREFIX}/qualify.py",
...
PRODUCT_BLOBS = {
    "mycelix-workspace/mycelix-core/libs/psi-query-credit-sqlite-spend/Cargo.toml": "0a792be2fddf67152105bc071d51d5dd2af5920b",
    "mycelix-workspace/mycelix-core/libs/psi-query-credit-sqlite-spend/README.md": "703255d5f6e7e63c13fafd69a280302dc551fc37",
    "mycelix-workspace/mycelix-core/libs/psi-query-credit-sqlite-spend/src/lib.rs": "e2e9b297f4692b7444574ea5f30a432c6df246c6",
}
B3B1_BLOBS = {
...
LOCK_SCHEMA = "mycelix.psi.002b3b2aq.lock.v0.1"
COMMANDS = (
    ("cargo", "fmt", "--check", "--all"),
...
def verify_checkout(repo: pathlib.Path) -> tuple[str, str]:
    ...
    for path, oid in {**PRODUCT_BLOBS, **B3B1_BLOBS, **B3A_BLOBS}.items():
...
def expected_lock(repo: pathlib.Path) -> dict[str, Any]:
    return {
        ...
        "authority": {
            "scope": "SqliteProfileExecutionOnly",
...
def probes(repo: pathlib.Path) -> dict[str, Any]:
    cargo = read_product(repo, "mycelix-workspace/mycelix-core/libs/psi-query-credit-sqlite-spend/Cargo.toml").decode()
    source = read_product(repo, "mycelix-workspace/mycelix-core/libs/psi-query-credit-sqlite-spend/src/lib.rs").decode()
    if 'rusqlite = { version = "=0.39.0"' not in cargo:
        raise QualificationError("rusqlite pin mismatch")
    required = (
        'SQLITE_SPEND_PROFILE_V1',
        'SQLITE_BUSY_TIMEOUT_MS: u64 = 5_000',
        'SQLITE_SYNCHRONOUS_FULL_CODE: i64 = 2',
        'PRAGMA journal_mode=WAL',
        'pragma_update(None, "synchronous", SQLITE_SYNCHRONOUS_FULL_CODE)',
        'pragma_query_value(None, "busy_timeout"',
        'pragma_query_value(None, "user_version"',
        'TransactionBehavior::Immediate',
        'INSERT OR IGNORE INTO query_token_spends_v1',
        'transaction.commit()?;',
        'SqliteFullSyncConsumedQueryTokenV1',
        'sqlite_atomic_single_use_established',
        'sqlite_full_sync_profile_established',
        ...
        'persisted_schema_has_no_raw_token_column',
    )
    for token in required:
        if token not in source:
            raise QualificationError(f"required SQLite theorem token absent: {token}")
    for forbidden in ('raw_token', 'token_bytes', 'privacy_pass_token BLOB', 'query_credit_granted(&self) -> bool { true'):
        if forbidden in source:
            raise QualificationError(f"forbidden authority/storage token present: {forbidden}")
    commit_pos = source.find('transaction.commit()?;')
    positive_pos = source.find('Ok(SqliteFullSyncConsumedQueryTokenV1', commit_pos)
    if commit_pos < 0 or positive_pos < 0 or positive_pos <= commit_pos:
        raise QualificationError("commit-before-positive ordering not found")
    count = source.count("#[test]")
    if count != EXPECTED_TEST_COUNT:
        raise QualificationError(f"test count mismatch: {count}")
    return {"registered_tests": count, "probe_profile": "psi-002b3b2aq-v0.1", "rusqlite_pin": "0.39.0"}

def materialize(repo: pathlib.Path, tmp: pathlib.Path) -> pathlib.Path:
    for crate_name, blobs in (
        ("psi-privacy-pass-credit-core", B3A_BLOBS),
        ("psi-query-credit-spend-core", B3B1_BLOBS),
    ...
    return tmp / "psi-query-credit-sqlite-spend"

def execute(command: tuple[str, ...], cwd: pathlib.Path) -> dict[str, Any]:
    result = run(list(command), cwd)
    record = {"argv": list(command), "returncode": result.returncode, "stdout_sha256": sha256(result.stdout.encode()), "stderr_sha256": sha256(result.stderr.encode())}
    if result.returncode:
...
def qualify(repo: pathlib.Path, receipt: pathlib.Path) -> dict[str, Any]:
    ...
    repo = repo.resolve(); receipt = receipt.resolve()
    try:
    ...
    with tempfile.TemporaryDirectory(prefix="psi-002b3b2aq-") as directory:
...
def main() -> int:
    ...
    try:
        ...
        print(f"PSI-002B3B2AQ FAIL: {exc}")
        return 1
