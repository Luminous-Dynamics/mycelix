...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(OpEntry::CreateEntry { app_entry, action }) => match app_entry {
            ...
            EntryTypes::RestitutionAgreement(a) => {
                // from_did/to_did are deliberately third-party (the
                // facilitator brokers restitution between HarmDoer and
                // HarmReceiver; neither is expected to be the committer).
                // Bind instead to the parent circle's facilitator, using
                // the must_get_valid_record cross-entry pattern used
                // elsewhere in this workspace (e.g. robotics-dispatch's
                // validate_dispatch_order).
                let circle_record = must_get_valid_record(a.circle_hash.clone())?;
                let circle: RestorativeCircle = circle_record
                    .entry()
                    .to_app_option()
                    .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
                    .ok_or(wasm_error!(WasmErrorInner::Guest(
                        "Parent restorative circle not found".into()
                    )))?;
                let expected = format!("did:mycelix:{}", action.author);
                if circle.facilitator != expected {
                    return Ok(ValidateCallbackResult::Invalid(
                        "restitution agreement must be committed by the circle's facilitator"
                            .into(),
                    ));
                }
                validate_restitution(&a)
            }
        },
