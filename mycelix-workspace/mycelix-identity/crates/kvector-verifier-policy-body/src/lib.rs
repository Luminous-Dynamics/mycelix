...
//! Backend-neutral verifier-policy body theorem for Identity V2 K-vector evidence.
//!
//! This crate freezes the *contents* of one verifier policy and its deterministic
//! digest. It does not decide who may publish a policy, whether a policy is current,
//! or whether a signature/proof is valid.
//!
//! Security split:
//!
//! - #225 owns the exact proof-statement theorem;
//! - #235 owns the exact verifier-signed verification-record body;
//! - this crate binds one record body to one exact verifier/backend/circuit policy;
//! - #246 owns authoritative policy lineage, supersession, revocation and currentness;
//! - #244 owns authoritative verifier-key lineage;
//! - only a later adapter combining real signature success + current key evidence +
//!   current accepted policy evidence may construct positive verification authority.
...
/// Canonical body of one V2 verifier policy.
///
/// V2 deliberately describes one exact verifier key/backend/circuit tuple per policy
/// body. Multiple accepted tuples should be represented as multiple independently
/// versioned policy bodies rather than an unordered allowlist whose canonical hashing
/// would become a second protocol.
...
/// Derive the canonical digest referenced by #235's `verification_policy_sha256`.
///
/// The exact #225 proof-statement domain is included in the policy transcript so the
/// same backend/circuit metadata cannot be silently reused for a different theorem.
...
/// Check static compatibility between one #235 verification-record assertion and one
/// policy body.
///
/// Success means only that the record *claims* the tuple and policy digest described
/// by this body and that its verification/validity interval stays inside the policy's
/// static interval. It does not establish policy authority/currentness or signature
/// validity.
...
mod tests {
    ...
    const POLICY_DIGEST_VECTOR: [u8; SHA256_DIGEST_LEN_V2] = [
        0x6c, 0x2d, 0x3f, 0x00, 0x20, 0x61, 0xe6, 0xf7,
        0xe2, 0x21, 0x2e, 0x65, 0xe2, 0x78, 0xcd, 0xe6,
        0x58, 0x1b, 0xfc, 0x3d, 0x21, 0xaa, 0x95, 0xc7,
        0x81, 0x20, 0xd8, 0x8f, 0xab, 0x03, 0x7b, 0x43,
    ];
    ...
    fn policy_digest_vector_is_frozen() {
        assert_eq!(derive_kvector_verifier_policy_digest_v2(policy()), Ok(POLICY_DIGEST_VECTOR));
    }
