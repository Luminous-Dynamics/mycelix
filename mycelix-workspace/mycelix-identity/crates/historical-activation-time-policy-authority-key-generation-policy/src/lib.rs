...
//! Exact authority-domain-scoped policy-authority key material for historical time-policy transitions.
//!
...
//! bootstrapped from a candidate clock.
//!
//! Success here establishes only a domain-scoped material identity. It does not establish that the
//! named signer generation is authorized, current, rooted, or cryptographically authentic for any
//! transition. Those are later, independent promotion steps.
...
#[derive(Debug)]
pub struct ExactHistoricalActivationTimePolicyAuthorityKeyGenerationV2 {
...
impl ExactHistoricalActivationTimePolicyAuthorityKeyGenerationV2 {
...
pub fn derive_exact_historical_activation_time_policy_authority_key_generation_v2(
    authority_domain: &QualifiedIdentityAuthorityDomainV2,
    body: HistoricalActivationTimePolicyAuthorityKeyGenerationV2<'_>,
) -> Result<
...
> {
    ...
    Ok(ExactHistoricalActivationTimePolicyAuthorityKeyGenerationV2 {
...
pub fn validate_exact_policy_authority_key_generation_against_transition_v2(
...
mod tests {
    ...
    fn exact_generation_binds_to_transition() {
        let domain = domain();
        let generation = derive_exact_historical_activation_time_policy_authority_key_generation_v2(
            &domain,
            body(),
        )
        .unwrap();
        ...
        .unwrap();
        assert_eq!(
            validate_exact_policy_authority_key_generation_against_transition_v2(&generation, &transition),
            Ok(())
        );
    }
    ...
    fn same_material_in_other_domain_has_different_generation_identity() {
        ...
        let first = derive_exact_historical_activation_time_policy_authority_key_generation_v2(
            &first_domain,
            body(),
        )
        .unwrap();
        ...
        .unwrap();
        let second = derive_exact_historical_activation_time_policy_authority_key_generation_v2(
            &second_domain,
            body(),
        )
        .unwrap();
    ...
    #[test]
    fn substituted_public_key_material_does_not_bind_to_transition() {
        let domain = domain();
        let original = derive_exact_historical_activation_time_policy_authority_key_generation_v2(
            &domain,
            body(),
        )
        .unwrap();
        let transition = prepare_time_policy_adoption_transition_v2(
            ...
            None,
            HistoricalActivationTimePolicyAuthoritySignerV2 {
                policy_authority_id: original.policy_authority_id(),
                policy_authority_key_id: original.policy_authority_key_id(),
                policy_authority_key_generation_sha256: original.generation_digest_sha256(),
                algorithm: original.algorithm(),
            },
        )
        .unwrap();

        let mut substituted_key = POLICY_AUTHORITY_PUBLIC_KEY;
        substituted_key[0] ^= 0x01;
        let substituted_body = HistoricalActivationTimePolicyAuthorityKeyGenerationV2 {
            public_key_bytes: &substituted_key,
            ..body()
        };
        let substituted =
        ...
        assert_ne!(
            original.generation_digest_sha256(),
            substituted.generation_digest_sha256()
        );
        assert_eq!(
            validate_exact_policy_authority_key_generation_against_transition_v2(
                &substituted,
                &transition,
            )
            .unwrap_err(),
            HistoricalActivationTimePolicyAuthorityKeyGenerationErrorV2::TransitionAuthorityKeyGenerationMismatch
        );
    }

    #[test]
    fn invalid_key_shape_zero_material_and_zero_generation_fail_closed() {
        let domain = domain();
        let bad_key = [0x66; 31];
        let invalid = HistoricalActivationTimePolicyAuthorityKeyGenerationV2 {
            public_key_bytes: &bad_key,
            ..body()
        };
        assert_eq!(
            derive_exact_historical_activation_time_policy_authority_key_generation_v2(
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            HistoricalActivationTimePolicyAuthorityKeyGenerationErrorV2::PublicKeyAllZero
        );
    ...
    #[test]
    fn exact_generation_fields_are_private() {
        let source = include_str!("lib.rs");
        let start = source
            .index("pub struct ExactHistoricalActivationTimePolicyAuthorityKeyGenerationV2")
            .unwrap();
        let end = source[start..]
            .index("impl ExactHistoricalActivationTimePolicyAuthorityKeyGenerationV2")
            .unwrap()
            + start;
