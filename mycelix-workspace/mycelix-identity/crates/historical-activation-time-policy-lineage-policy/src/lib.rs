...
/// Qualify one complete observed prepared-transition set as a single topology/state lineage.
///
...
> {
    ...
    if ordered[0].transition_generation() != 1 {
    ...
    let terminal = *ordered.last().expect("non-empty checked above");
