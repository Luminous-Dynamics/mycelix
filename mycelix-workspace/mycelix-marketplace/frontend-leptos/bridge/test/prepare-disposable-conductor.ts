...
type Actor = {
  prefix: "SELLER" | "BUYER" | "ARBITRATOR";
...
async function main(): Promise<void> {
  const configPath = process.env.MARKETPLACE_DISPOSABLE_CONFIG;
  if (!configPath) throw new Error("MARKETPLACE_DISPOSABLE_CONFIG is required");
  const config = JSON.parse(await readFile(configPath, "utf8")) as Config;
  if (!config.actors.length) throw new Error("at least one actor is required");

  const admin = await AdminWebsocket.connect({ url: new URL(config.admin_url) });
  const lines: string[] = [];
  try {
    for (const actor of config.actors) {
      ...
      lines.push(`MARKETPLACE_${actor.prefix}_ADMIN_URL=${config.admin_url}`);
