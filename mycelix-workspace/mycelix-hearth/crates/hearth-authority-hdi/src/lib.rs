...
pub fn require_fresh_active_membership_from_chain(
    ...
    // Holochain 0.6 uses the builder-form ChainFilter API. The hash-bounded
    // range walks backward from the transition's previous action to the claimed
    // membership action, which is the deterministic window the pure kernel
    // expects for freshness evaluation.
    let activity = must_get_agent_activity(
        actor.clone(),
        ChainFilter::new(previous_action).until(claimed_membership_hash.clone()),
    )?;
