...
pub fn StatsExplorer(node_id: String) -> impl IntoView {
    ...
    let challenges = vec![
        Challenge {
            instruction: "Make the mean equal to 10",
            check: |mean, _, _| (mean - 10.0).abs() < 0.5,
            hint: "The mean is the sum divided by the count. Adjust values so they average to 10.",
        },
        Challenge {
            instruction: "Make the standard deviation less than 1",
            check: |_, _, sd| sd < 1.0,
            hint: "Standard deviation measures spread. Make all values very close together.",
        },
        Challenge {
            instruction: "Make the median greater than the mean",
            check: |mean, median, _| median > mean + 0.5,
            hint: "Add some very low values to pull the mean down while keeping the middle values high.",
        },
        Challenge {
            instruction: "Make the range exactly 10",
            check: |_, _, _| false,
            hint: "Range = max - min. Set the highest value 10 more than the lowest.",
        }, // Note: we'll check range separately
    ];
