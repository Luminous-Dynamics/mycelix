...
impl SimulatedEngine {
    ...
    fn tick(&mut self) -> ConsciousnessState {
        ...
        let free_energy =
            (1.0 - consciousness as f64 * 0.7 + (self.rand() - 0.5) * 0.1).clamp(0.1, 1.5) as f32;
...
impl SporeEngineBridge {
    fn try_new() -> Option<Self> {
                        ...
                        if msg_type == "cycle" {
                            if let Ok(result) = js_sys::Reflect::get(&e.data(), &"result".into()) {
                                ...
                                let new_state = ConsciousnessState {
                                    ...
                                    cycle_count: js_sys::Reflect::get(
                                        &result,
                                        &"cycle_count".into(),
                                    )
                                    .ok()
        ...
        web_sys::console::log_1(
            &"[Praxis] Spore Web Worker initialized — real consciousness active".into(),
        );
...
pub fn ConsciousnessProvider(children: Children) -> impl IntoView {
    ...
    let spore_available =
        js_sys::Reflect::get(&web_sys::window().unwrap(), &"__SPORE_AVAILABLE".into())
            .ok()
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
    ...
    if spore_available {
        ...
        web_sys::console::log_1(&"[Praxis] Spore WASM not found \u{2014} using simulation".into());
    }
