# Pulse Workspace Vision — Mail + Chat + Meet North Star

> **Status (added 2026-07-25, renamed from `WORKSPACE_PLAN.md`): North-star architecture, not a current implementation roadmap.**
> Pulse Mail is the only supported product surface today, and `PULSE_READINESS_PLAN.md` /
> `PULSE_NEXT_SESSION_PLAN_2026-07-18.md` are the canonical roadmaps for it — this document
> does not supersede them and, as of this note, has zero implemented checkboxes. Chat and
> Meet below describe intended future expansion **after** Mail reaches its defined maturity
> gates (see `MASTER_ROADMAP.md`'s Pulse row), not committed release scheduling. The phase/
> version numbering below (v0.3.0, v0.4.0, …) is conceptual sequencing, not a version plan
> tied to Pulse's actual releases. Read this as "what a unified Mail+Chat+Meet substrate
> could look like, and why it's plausible given existing reusable infrastructure" — not
> as work in flight.
