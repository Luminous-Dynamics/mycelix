#!/usr/bin/env python3
"""Execute Prism Wave 12's post-admission release gates and publish exact evidence.

This is the live complement to `verify-wave12-static.py`. It is intentionally a
...
def require_clean_source(git: str) -> None:
    ...
    if status:
        raise LiveGateError(f"Wave 12 live gates require a clean source tree:\n{status}")
...
def command_specs(
    ...
    return (
        CommandSpec(
            "wave12-static",
            (python, "scripts/verify-wave12-static.py", "--check", "security/wave12-static-evidence.json"),
        ),
        CommandSpec(
...
def require_no_checkpoint_change(
    ...
    if list(before) != list(after):
        raise LiveGateError(
            "Wave 12 post-admission verification changed the release journal"
        )
...
def rename_noreplace(staging: Path, output: Path) -> None:
    if os.name != "posix":
        raise LiveGateError("Wave 12 live publication requires Linux renameat2")
...
def main() -> int:
    ...
    try:
        ...
        if not (ROOT / "flake.lock").is_file():
            raise LiveGateError("Wave 12 live gates require a committed flake.lock")
        ...
        staging = Path(tempfile.mkdtemp(prefix=".prism-wave12-live-", dir=output_parent))
        try:
            commands = execute(
                command_specs(
                    inputs, bwrap, python, cargo, nix, channel, head_output, audit_output,
                    recovery_output, transparency_output
                ),
                environment,
                staging,
            )
            ...
            if source_after != source_before:
                raise LiveGateError("source identity changed during Wave 12 live gates")
            ...
            transparency_identity = file_identity(admitted_path(str(transparency_output), directory=False))
            receipt = {
                "schema_version": SCHEMA_VERSION,
                "campaign": "prism-hardening-wave-12-live",
                "status": "pass",
        ...
        print(f"Wave 12 live gates failed: {error}", file=sys.stderr)
        return 1
