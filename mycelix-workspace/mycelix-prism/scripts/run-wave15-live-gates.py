#!/usr/bin/env python3
"""Execute Prism Wave 15's post-admission release gates and publish exact evidence.

This is the live complement to `verify-wave15-static.py`. It is intentionally a
...
def require_clean_source(git: str) -> None:
    ...
    if status:
        raise LiveGateError(f"Wave 15 live gates require a clean source tree:\n{status}")
...
def parse_inputs(arguments: argparse.Namespace) -> Inputs:
    ...
    require_non_overlapping(
        (
            ...
            ("Wave 15 continuity config", inputs.continuity_config),
...
def command_specs(
    ...
    return (
        CommandSpec(
            "wave15-static",
            (python, "scripts/verify-wave15-static.py", "--check", "security/wave15-static-evidence.json"),
        ),
        CommandSpec(
        ...
        CommandSpec(
            "wave15-continuity",
            (python, "scripts/run-wave15-continuity-gates.py", "--config", str(inputs.continuity_config)),
        ),
        CommandSpec(
...
def require_no_checkpoint_change(
    ...
    if list(before) != list(after):
        raise LiveGateError(
            "Wave 15 post-admission verification changed the release journal"
        )
...
def rename_noreplace(staging: Path, output: Path) -> None:
    if os.name != "posix":
        raise LiveGateError("Wave 15 live publication requires Linux renameat2")
...
def main() -> int:
    ...
    try:
        ...
        if not (ROOT / "flake.lock").is_file():
            raise LiveGateError("Wave 15 live gates require a committed flake.lock")
        ...
        staging = Path(tempfile.mkdtemp(prefix=".prism-wave15-live-", dir=output_parent))
        try:
            commands = execute(
                command_specs(
            ...
            if source_after != source_before:
                raise LiveGateError("source identity changed during Wave 15 live gates")
            ...
            receipt = {
                ...
                "artifacts": {
                    ...
                    "wave15_continuity_config": file_identity(inputs.continuity_config),
        ...
        print(f"Wave 15 live gates failed: {error}", file=sys.stderr)
        return 1
