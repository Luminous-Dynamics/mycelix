...
import { c as create_ssr_component, a as subscribe, o as onDestroy, b as escape, e as each, n as null_to_empty } from "../../../chunks/ssr.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let $meshHealth, $$unsubscribe_meshHealth;
  const stats = writable({
    totalNodes: 156,
    onlineNodes: 142,
    totalPeers: 1234,
  ...
  $$unsubscribe_meshHealth = subscribe(meshHealth, (value) => $meshHealth = value);
  let meshTransport = "unknown";
  ...
  })}</div></div></div></div>  <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6" data-svelte-h="svelte-svae7x"><div class="bg-gray-800 rounded-lg border border-gray-700 p-4"><h3 class="text-lg font-semibold mb-3 flex items-center gap-2"><span>🌍</span> Geographic Distribution</h3> <div class="space-y-2 text-sm"><div class="flex justify-between"><span class="text-gray-400">North America</span> <span>42 nodes</span></div> <div class="flex justify-between"><span class="text-gray-400">Europe</span> <span>38 nodes</span></div> <div class="flex justify-between"><span class="text-gray-400">Asia Pacific</span> <span>31 nodes</span></div> <div class="flex justify-between"><span class="text-gray-400">Other</span> <span>31 nodes</span></div></div></div> <div class="bg-gray-800 rounded-lg border border-gray-700 p-4"><h3 class="text-lg font-semibold mb-3 flex items-center gap-2"><span>⚡</span> DHT Performance</h3> <div class="space-y-2 text-sm"><div class="flex justify-between"><span class="text-gray-400">Gossip Interval</span> <span class="text-green-400">2s</span></div> <div class="flex justify-between"><span class="text-gray-400">Validation Rate</span> <span class="text-green-400">98.7%</span></div> <div class="flex justify-between"><span class="text-gray-400">Sync Lag</span> <span class="text-blue-400">&lt;500ms</span></div> <div class="flex justify-between"><span class="text-gray-400">Partition Events</span> <span class="text-green-400">0</span></div></div></div> <div class="bg-gray-800 rounded-lg border border-gray-700 p-4"><h3 class="text-lg font-semibold mb-3 flex items-center gap-2"><span>🛡️</span> Byzantine Status</h3> <div class="space-y-2 text-sm"><div class="flex justify-between"><span class="text-gray-400">BFT Threshold</span> <span class="text-green-400">34%</span></div> <div class="flex justify-between"><span class="text-gray-400">Current Tolerance</span> <span class="text-green-400">36.2%</span></div> <div class="flex justify-between"><span class="text-gray-400">Detected Bad Actors</span> <span class="text-yellow-400">3</span></div> <div class="flex justify-between"><span class="text-gray-400">Cartel Detection</span> <span class="text-green-400">Active</span></div></div></div></div>  <div class="mt-6 bg-gray-800 rounded-lg border border-gray-700"><div class="p-4 border-b border-gray-700 flex justify-between items-center"><h2 class="text-lg font-semibold" data-svelte-h="svelte-1vvgvua">Mesh Bridge (Offline Relay)</h2> <span class="${"text-xs px-2 py-1 rounded-full " + escape(
    $meshHealth.status === "running" ? "bg-green-500/20 text-green-400" : $meshHealth.status === "offline" ? "bg-red-500/20 text-red-400" : "bg-gray-500/20 text-gray-400",
    true
  )}">${escape($meshHealth.status)}</span></div> ${$meshHealth.status === "running" ? `<div class="p-4"><div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4"><div class="bg-gray-700/50 rounded p-3"><p class="text-xs text-gray-400 uppercase" data-svelte-h="svelte-1obeaoq">Sent</p> <p class="text-xl font-bold text-blue-400">${escape($meshHealth.metrics.messages_sent)}</p></div> <div class="bg-gray-700/50 rounded p-3"><p class="text-xs text-gray-400 uppercase" data-svelte-h="svelte-1i4b2w5">Received</p> <p class="text-xl font-bold text-green-400">${escape($meshHealth.metrics.messages_received)}</p></div> <div class="bg-gray-700/50 rounded p-3"><p class="text-xs text-gray-400 uppercase" data-svelte-h="svelte-rmye7o">Peers Seen</p> <p class="text-xl font-bold text-purple-400">${escape($meshHealth.metrics.peers_seen)}</p></div> <div class="bg-gray-700/50 rounded p-3"><p class="text-xs text-gray-400 uppercase" data-svelte-h="svelte-152c0ru">Dropped</p> <p class="${"text-xl font-bold " + escape(
    $meshHealth.metrics.fragments_dropped > 0 ? "text-yellow-400" : "text-gray-400",
    true
  )}">${escape($meshHealth.metrics.fragments_dropped)}</p></div></div> <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm"><div><p class="text-xs text-gray-400" data-svelte-h="svelte-xz0i41">Transport</p> <p class="text-gray-200">${escape(meshTransport)}</p></div> <div><p class="text-xs text-gray-400" data-svelte-h="svelte-1u5ykkg">Poll Cycles</p> <p class="text-gray-200">${escape($meshHealth.metrics.poll_cycles)}</p></div> <div><p class="text-xs text-gray-400" data-svelte-h="svelte-7de8ca">Last Send</p> <p class="text-gray-200">${escape(formatMeshTime($meshHealth.metrics.last_send_at))}</p></div> <div><p class="text-xs text-gray-400" data-svelte-h="svelte-jyfvqj">Last Receive</p> <p class="text-gray-200">${escape(formatMeshTime($meshHealth.metrics.last_recv_at))}</p></div></div> ${$meshHealth.metrics.connection_failures > 0 ? `<div class="mt-3 p-2 bg-yellow-900/20 border border-yellow-800/50 rounded text-sm text-yellow-300">${escape($meshHealth.metrics.connection_failures)} conductor connection failure${escape($meshHealth.metrics.connection_failures !== 1 ? "s" : "")}</div>` : ``}</div>` : `${$meshHealth.status === "offline" ? `<div class="p-4 text-sm text-gray-400" data-svelte-h="svelte-195864t"><p>Mesh bridge is not running. This is normal if you haven&#39;t deployed mesh hardware.</p> <p class="mt-2">To enable: set <code class="bg-gray-700 px-1 rounded">MESH_TRANSPORT=lora</code> or <code class="bg-gray-700 px-1 rounded">MESH_TRANSPORT=batman</code> in your deployment config.</p></div>` : `<div class="p-4 text-sm text-gray-400" data-svelte-h="svelte-1tg5o1y">Checking mesh bridge status...</div>`}`}</div>  <footer class="mt-8 text-center text-gray-500 text-sm" data-svelte-h="svelte-9d83ds"><p>Mycelix Network Monitor v0.1.0 • Holochain 0.6.0</p> <p class="mt-1">Distributed Infrastructure for the Civilizational OS</p></footer></main> </div>`;
});
export {
  Page as default
};
