...
import { v as volatilityData, e as enrichedItems, h as hasBasket, p as purchasingPowerIndex, b as basketHistory, o as oracleLoading, c as basketIndexData, C as CANONICAL_ITEMS, R as RESILIENCE_BASKET_NAME, g as getCommunityConfig } from "../../../chunks/value-basket.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-8mhws2_START -->${$$result.title = `<title>Value Anchor | Mycelix Observatory</title>`, ""}<!-- HEAD_svelte-8mhws2_END -->`, ""} <div class="text-white"><header class="bg-gray-800/50 border-b border-gray-700 px-4 py-2"><div class="container mx-auto flex justify-between items-center"><div class="flex items-center gap-2"><span class="text-xl" data-svelte-h="svelte-wa8958">⚖</span> <div><h1 class="text-lg font-bold" data-svelte-h="svelte-ddds6d">Value Anchor</h1> <p class="text-xs text-gray-400">${escape(RESILIENCE_BASKET_NAME)}</p></div></div> <div class="flex items-center gap-4"> <div class="text-xs">${$conductorStatus$ === "connected" ? `<span class="text-green-400" data-svelte-h="svelte-1op8tqr">DHT Live</span>` : `<span class="text-yellow-400" data-svelte-h="svelte-1efz3i0">Demo Mode</span>`}</div>  ${$volatilityData ? `<div class="text-right"><p class="text-xs text-gray-400" data-svelte-h="svelte-86kal">Volatility</p> <p class="${"text-sm font-bold " + escape(volatilityColor, true)}">${escape($volatilityData.recommended_tier)}
    ...
    return `<div class="p-3 bg-gray-700/50 rounded-lg flex justify-between items-center"><div class="flex items-center gap-3"> <div class="${"w-2 h-2 rounded-full " + escape(
      ...
      0
    )}></div> <div><p class="font-medium text-sm">${escape(item.name)}</p> <p class="text-xs text-gray-400">${escape(item.unit)}</p> </div></div> <div class="flex items-center gap-4">${item.source === "consensus" ? `<div class="text-right"><span class="text-sm font-bold text-green-400">${escape(item.consensus_price?.toFixed(2))} TEND</span> <p class="text-xs text-gray-500">${escape(item.reporter_count)} reporters | SD ${escape(item.std_dev.toFixed(3))} |
                      <span${add_attribute(
      "class",
      ...
      0
                      ...
                      </span></p> </div>` : `<span class="text-sm font-bold text-yellow-400">${escape(item.price_tend.toFixed(2))} TEND</span>`} <button class="text-xs text-blue-400 hover:text-blue-300" data-svelte-h="svelte-37tepb">report</button></div> </div>`;
