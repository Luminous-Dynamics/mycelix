...
//! PSI-002B3B2A — SQLite WAL/FULL atomic spend backend.
//!
//! This backend persists only canonical spend-key digests and audit metadata.
//! It never accepts raw Privacy Pass token bytes and never grants query credit.

#![forbid(unsafe_code)]

use psi_query_credit_spend_core::QueryCreditSpendKeyV1;
use rusqlite::{params, Connection, TransactionBehavior};
use serde::Serialize;
use std::{path::Path, sync::Mutex, time::Duration};

pub const SQLITE_SPEND_PROFILE_V1: &str = "sqlite-wal-full-query-token-spend-v1";
pub const SQLITE_SCHEMA_VERSION_V1: i64 = 1;
pub const SQLITE_BUSY_TIMEOUT_MS: u64 = 5_000;
pub const SQLITE_SYNCHRONOUS_FULL_CODE: i64 = 2;

const SCHEMA_SQL: &str = r#"
CREATE TABLE IF NOT EXISTS query_credit_store_meta_v1 (
    key TEXT PRIMARY KEY NOT NULL,
    value TEXT NOT NULL
) STRICT;
CREATE TABLE IF NOT EXISTS query_token_spends_v1 (
...
#[derive(Debug)]
pub struct SqliteFullSyncSpendStoreV1 {
    connection: Mutex<Connection>,
    store_instance_id: String,
    sqlite_runtime_version: String,
}

impl SqliteFullSyncSpendStoreV1 {
    pub fn open(
        ...
        if existing_version != 0 && existing_version != SQLITE_SCHEMA_VERSION_V1 {
            return Err(SqliteSpendError::SchemaVersionMismatch(existing_version));
        }

        connection.execute_batch(SCHEMA_SQL)?;
        if existing_version == 0 {
            connection.pragma_update(None, "user_version", SQLITE_SCHEMA_VERSION_V1)?;
        }
        verify_full_profile(&connection)?;

        let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        transaction.execute(
            "INSERT OR IGNORE INTO query_credit_store_meta_v1(key, value) VALUES('store_instance_id', ?1)",
            params![&store_instance_id],
        )?;
        let persisted: String = transaction.query_row(
            "SELECT value FROM query_credit_store_meta_v1 WHERE key='store_instance_id'",
            [],
            |row| row.get(0),
        )?;
    ...
    pub fn consume_once(
    ...
    ) -> Result<SqliteFullSyncConsumedQueryTokenV1, SqliteSpendError> {
        ...
        let changed = transaction.execute(
            "INSERT OR IGNORE INTO query_token_spends_v1(
                spend_key_sha256, policy_sha256, token_sha256, requested_identifier_count
             ) VALUES(?1, ?2, ?3, ?4)",
            params![
    ...
    pub fn consumed_count(&self) -> Result<u64, SqliteSpendError> {
        ...
        let count: i64 = connection.query_row(
            "SELECT COUNT(*) FROM query_token_spends_v1",
            [],
            |row| row.get(0),
        )?;
...
pub struct SqliteFullSyncConsumedQueryTokenV1 {
...
impl SqliteFullSyncConsumedQueryTokenV1 {
    ...
    pub const fn sqlite_atomic_single_use_established(&self) -> bool {
        true
    }

    pub const fn sqlite_full_sync_profile_established(&self) -> bool {
        true
    }
    ...
    pub const fn global_store_uniqueness_established(&self) -> bool {
        false
    }

    pub const fn hardware_power_loss_durability_established(&self) -> bool {
        false
    }
...
fn verify_full_profile(connection: &Connection) -> Result<(), SqliteSpendError> {
    ...
    let schema_version: i64 =
        connection.pragma_query_value(None, "user_version", |row| row.get(0))?;
    if schema_version != SQLITE_SCHEMA_VERSION_V1 {
...
mod tests {
    ...
    use psi_query_credit_spend_core::derive_spend_key_v1;
    ...
    fn key(token_byte: &str) -> QueryCreditSpendKeyV1 {
        let policy = policy();
        ...
        derive_spend_key_v1(&policy, &challenge, &observation).unwrap()
    }
    ...
    fn exact_wal_full_profile_and_first_commit() {
        ...
        let store = SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap();
        let positive = store.consume_once(&key("33")).unwrap();
    ...
    fn replay_fails_after_close_and_reopen() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("spends.sqlite3");
        let spend_key = key("33");
        {
            let store = SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap();
            store.consume_once(&spend_key).unwrap();
        }
        let reopened = SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap();
        assert_eq!(
            reopened.consume_once(&spend_key),
            Err(SqliteSpendError::Replay)
        );
    }
    ...
    fn two_connections_racing_one_database_have_one_winner() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("spends.sqlite3");
        let left = Arc::new(SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap());
        let right = Arc::new(SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap());
    ...
    #[test]
    fn distinct_tokens_each_commit_once() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("spends.sqlite3");
        let store = SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap();
        assert!(store.consume_once(&key("33")).is_ok());
        assert!(store.consume_once(&key("55")).is_ok());
    ...
    fn wrong_store_instance_identity_is_rejected() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("spends.sqlite3");
        drop(SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap());
        assert_eq!(
            SqliteFullSyncSpendStoreV1::open(&path, "other-store").unwrap_err(),
    ...
    fn schema_version_mismatch_is_rejected() {
        ...
        connection.pragma_update(None, "user_version", 2_i64).unwrap();
        drop(connection);
        assert_eq!(
            SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap_err(),
            SqliteSpendError::SchemaVersionMismatch(2)
        );
    }

    #[test]
    fn persisted_schema_has_no_raw_token_column() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("spends.sqlite3");
        drop(SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary").unwrap());
        let connection = Connection::open(&path).unwrap();
        let mut statement = connection
            .prepare("SELECT name FROM pragma_table_info('query_token_spends_v1') ORDER BY cid")
            .unwrap();
    ...
    fn positive_keeps_higher_authority_false() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("spends.sqlite3");
        let positive = SqliteFullSyncSpendStoreV1::open(&path, "contacts-primary")
            .unwrap()
            .consume_once(&key("33"))
            .unwrap();
        ...
        assert!(json.contains(SQLITE_SPEND_PROFILE_V1));
    }
}
