...
pub enum GitHubVerifierExecutorErrorV1 {
    ...
    InvalidProcessGroupId { purpose: GitHubVerifierStepPurposeV1 },
    ProcessGroupKillFailed { purpose: GitHubVerifierStepPurposeV1 },
    LingeringProcessGroup { purpose: GitHubVerifierStepPurposeV1 },
...
fn wait_for_step(
    ...
    loop {
        match poll_reader(&stdout_rx, &mut stdout_result, purpose, true) {
            Ok(Some(error)) => {
                return cleanup_failed_step(
                    child,
                    process_group_id,
                    purpose,
                    stdout_handle,
        ...
        match poll_reader(&stderr_rx, &mut stderr_result, purpose, false) {
            Ok(Some(error)) => {
                return cleanup_failed_step(
                    child,
                    process_group_id,
        ...
        if started.elapsed() >= timeout {
            return cleanup_failed_step(
                ...
                stdout_handle,
                stderr_handle,
                GitHubVerifierExecutorErrorV1::ProcessTimedOut { purpose },
            );
        }
            ...
            match child.try_wait() {
                ...
                Err(_) => {
                    return cleanup_failed_step(
                        ...
                        stdout_handle,
                        stderr_handle,
                        GitHubVerifierExecutorErrorV1::ChildPollFailed { purpose },
                    );
                }
            }
        }
    ...
    let status = status.expect("loop exits only with observed child status");
    let lingering_group = terminate_process_group_if_present(process_group_id, purpose)?;
    ...
    let stdout = map_reader_result(
        stdout_result.expect("loop exits only with stdout result"),
        purpose,
        true,
    )?;
    let stderr = map_reader_result(
        stderr_result.expect("loop exits only with stderr result"),
        purpose,
        false,
    )?;
    ...
    if lingering_group {
        return Err(GitHubVerifierExecutorErrorV1::LingeringProcessGroup { purpose });
    }
    Ok(StepOutput { stdout, stderr })
}

fn cleanup_failed_step(
    child: &mut Child,
    ...
    let group_cleanup = terminate_process_group_if_present(process_group_id, purpose);
    let direct_cleanup = kill_direct_child(child, purpose);
    let wait_result = child
        .wait()
        .map(|_| ())
        .map_err(|_| GitHubVerifierExecutorErrorV1::ChildWaitFailed { purpose });
    let readers_joined = stdout_handle.join().is_ok() && stderr_handle.join().is_ok();
...
fn kill_direct_child(
    ...
    match child.kill() {
        ...
        Err(_) => Err(GitHubVerifierExecutorErrorV1::ChildKillFailed { purpose }),
    }
}
...
fn terminate_process_group_if_present(
    process_group_id: u32,
    purpose: GitHubVerifierStepPurposeV1,
) -> Result<bool, GitHubVerifierExecutorErrorV1> {
    let process_group_id = i32::try_from(process_group_id)
        .map_err(|_| GitHubVerifierExecutorErrorV1::InvalidProcessGroupId { purpose })?;
    if process_group_id <= 0 {
        return Err(GitHubVerifierExecutorErrorV1::InvalidProcessGroupId { purpose });
    }

    // SAFETY: every verifier step is spawned with `process_group(0)`, making the
    // child's positive PID the dedicated process-group ID. POSIX/Linux `kill(2)`
    // interprets a negative PID as "signal this process group". The value is
    // range-checked above, and SIGKILL has no user-space handler semantics.
    let result = unsafe { kill(-process_group_id, LINUX_SIGKILL) };
    if result == 0 {
        return Ok(true);
    }
    let os_error = io::Error::last_os_error();
    if os_error.raw_os_error() == Some(LINUX_ESRCH) {
        Ok(false)
    } else {
        Err(GitHubVerifierExecutorErrorV1::ProcessGroupKillFailed { purpose })
    }
}
...
fn terminate_process_group_if_present(
    ...
    _purpose: GitHubVerifierStepPurposeV1,
) -> Result<bool, GitHubVerifierExecutorErrorV1> {
    Err(GitHubVerifierExecutorErrorV1::UnsupportedHostPlatform)
}
