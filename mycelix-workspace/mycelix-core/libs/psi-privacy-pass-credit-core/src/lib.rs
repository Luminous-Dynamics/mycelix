...
//! PSI-002B3A — RFC 9577/9578 structural one-time query-credit semantics.
//!
//! This crate implements no Privacy Pass cryptography. It freezes the exact
//! Mycelix policy/challenge bindings that a future qualified backend must verify
//! before an atomically consumed token may become a query credit.
//!
//! ```text
//! structural token/policy compatibility
...
impl QueryCreditPolicyV1 {
    ...
    /// 32-byte RFC 9577 redemption-context value for this exact Mycelix policy.
    ///
    /// This is a policy-derived context, not proof that an RFC implementation
    /// actually encoded or enforced it.
    ...
    pub fn challenge_binding(&self) -> Result<PrivacyPassChallengeBindingV1, QueryCreditStructuralFailure> {
...
/// Mycelix semantic binding for the RFC 9577 challenge fields.
///
/// This is not the RFC TLS-wire encoding; a concrete adapter must independently
/// reproduce RFC 9577 bytes and prove the token challenge digest matches them.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
/// Raw backend observation. All fields remain untrusted until a concrete RFC
/// backend and atomic replay store establish them.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
pub fn evaluate_redemption_structure_v1(
    ...
    let incompatible = |failure| QueryCreditStructuralEvaluationV1 {
        disposition: QueryCreditStructuralDisposition::Incompatible(failure),
    ...
    if challenge.token_type != policy.token_type || observation.token_type != policy.token_type {
        return Ok(incompatible(QueryCreditStructuralFailure::TokenTypeMismatch));
    }
    if challenge.issuer_name != policy.issuer_name || observation.issuer_name != policy.issuer_name {
        return Ok(incompatible(QueryCreditStructuralFailure::IssuerMismatch));
    }
    if observation.token_key_id_sha256 != policy.token_key_id_sha256 {
        return Ok(incompatible(QueryCreditStructuralFailure::TokenKeyMismatch));
    }
    if challenge.origin_info != vec![policy.service_domain.clone()]
        || observation.service_domain != policy.service_domain
    {
        return Ok(incompatible(QueryCreditStructuralFailure::ServiceDomainMismatch));
    }
    if observation.budget_epoch != policy.budget_epoch {
        return Ok(incompatible(QueryCreditStructuralFailure::BudgetEpochMismatch));
    }
    ...
    {
        return Ok(incompatible(QueryCreditStructuralFailure::ChallengeBindingMismatch));
    }
    if observation.requested_identifier_count > policy.max_identifiers_per_credit {
        return Ok(incompatible(QueryCreditStructuralFailure::IdentifierBudgetExceeded));
    }

    Ok(QueryCreditStructuralEvaluationV1 {
        disposition: QueryCreditStructuralDisposition::ReadyForBackendVerification,
        policy_sha256,
        challenge_binding_sha256: expected_challenge_sha256,
        token_sha256: observation.token_sha256.clone(),
        backend_profile: observation.backend_profile.clone(),
        backend_receipt_sha256: observation.backend_receipt_sha256.clone(),
        requested_identifier_count: observation.requested_identifier_count,
...
mod tests {
    ...
    fn structurally_matching_redemption_is_not_a_query_credit() {
        ...
        assert!(!result.anonymous_rate_limit_established());
