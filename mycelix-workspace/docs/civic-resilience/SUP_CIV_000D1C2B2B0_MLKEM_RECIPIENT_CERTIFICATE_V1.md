# SUP-CIV-000D1C2B2B0 r2 — ML-KEM Recipient Certificate / RID Binding Fixture v1
...
B2B0 r1 froze sound certificate/CA bytes but its verifier delegated validity checking to the runner's current wall clock. That makes qualification depend on scheduler start time rather than only on the frozen subject.

r2 reuses the exact same recipient and CA DER blobs and changes only the evidence harness so chain validation occurs at the frozen instant:
...
which is inside the frozen recipient-certificate validity interval.

```text
RunnerWallClock != EvidenceTime
QualificationHarnessRepair != CertificateSemanticChange
FixedVerificationTime != CurrentCertificateValidity
```

The fixed time establishes only that the frozen certificate chain validates at that exact instant. It is not a claim that the certificate is currently valid at runtime.

## Exact recipient certificate

The immutable recipient DER fixture is unchanged from r1:
...
The exact test CA is also unchanged:
...
2. recipient chain validation to the exact frozen test CA at Unix time `1790812800`;
...
A PASS may establish only the exact certificate/CA fixture bytes, X.509 profile, signature-chain validation at the frozen evidence instant, SPKI equality to the frozen deterministic ML-KEM public-key fixture, and RID/SKI equality. It does not establish current certificate validity, recipient authorization/currentness, production certificate issuance/trust, private-key custody, CMS interoperability, protected-storage access authority, legal compliance, municipal legitimacy or deployment readiness.
