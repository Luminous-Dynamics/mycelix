...
- [`COMPOSITION_RULES_V0.1.md`](./COMPOSITION_RULES_V0.1.md) — acyclic cross-domain qualification, exact evidence cuts, deterministic closure, authorization-to-execution validity, conserved-effect allocation, and no-double-counting rules.
...
3. Qualify cross-domain dependency direction, exact input-cut semantics, conservation/allocation rules, and deterministic closure.
4. Encode only the transport-neutral vocabulary required by the qualified corpus.
5. Property-test commitment, obligation, power, observation, reconciliation, allocation, replay/idempotency, and workflow-closure semantics.
6. Integrate a service-business path using existing Identity, Commerce, Finance, and future Accounting boundaries.
7. Add adversarial qualification before exposing a general Business Cockpit.
...
- a generic Business-owned allocation ledger that can overrule Finance, Supply Chain, workforce, or other owning-domain conservation semantics.
...
The state-machine gate additionally requires that the corpus preserve these non-substitutabilities:

- logical intent != execution attempt != source event != transport delivery;
...
The cross-domain composition gate additionally requires:

- dependency graphs are acyclic and cannot self-justify;
- one consequential qualification is bound to one coherent immutable evidence cut;
- the same profile + explicit time + exact cut produces the same consequential result;
- authorization cannot outlive or detach from the exact prerequisites and operation that justified it;
- one conserved real-world effect cannot be double-allocated merely because two workflows reference it;
- divisible/many-to-one allocations preserve exact quantities, subjects, and provenance;
- historical closure remains historical evidence while current projections still obey currentness requirements;
- reproducible qualification does not imply universal disclosure.

If any gate fails, the ontology is not frozen and runtime implementation must wait.
