...
//!
//! Extracted 2026-08-05 out of `mycelix-desci-core` into its own Cargo
//! workspace: `holochain_types` (via `rusqlite`) and `sqlx-sqlite` (an
//! optional dependency of `sqlx`, itself needed for `mycelix-desci-core`'s
//! Postgres authority backend) both declare `links = "sqlite3"` under
//! mutually incompatible version requirements (`^0.28` vs `^0.35`). Cargo
//! enforces a single version per `links` value across an entire workspace
//! Cargo.lock regardless of whether the relevant feature is actually
//! activated, so the two could never coexist in one workspace -- let alone
//! one linked binary -- no matter how the `holochain` feature was gated.
//! This crate is not currently wired to any binary; if `KnowledgeDhtStorage`
//! is needed again, it has to run as a separate process/service from
//! `mycelix-api`, not an in-process dependency.
...
use mycelix_desci_types::claims::{ClaimContent, DesciClaim, EpistemicPosition, EpistemicTier};
use mycelix_desci_types::{Error, Result};
...
#[async_trait]
impl mycelix_desci_types::storage::StorageBackend for KnowledgeDhtStorage {
...
mod tests {
    use super::*;
    use mycelix_desci_types::claims::ClaimContent;
    ...
    ///   cargo test -p mycelix-desci-holochain-bridge \
    ...
    async fn live_conductor_round_trip() {
        use mycelix_desci_types::storage::StorageBackend;
