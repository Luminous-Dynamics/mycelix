...
impl LEMCube {
    pub fn new(
        ...
        normative: NormativeAxis,
        materiality: MaterialityAxis,
    ) -> Self {
        Self {
            empirical,
            normative,
    ...
    pub fn personal_opinion() -> Self {
        Self::new(
            EmpiricalAxis::E0Null,
            NormativeAxis::N0Personal,
            MaterialityAxis::M0Ephemeral,
        )
    }
    ...
    pub fn cryptographic_proof() -> Self {
        Self::new(
            EmpiricalAxis::E3CryptographicallyProven,
            NormativeAxis::N3Axiomatic,
            MaterialityAxis::M3Foundational,
        )
    }
...
impl EpistemicFingerprint {
    ...
    pub fn confidence_score(&self) -> f64 {
        ...
        let type_weight = self
            .type_position
            .empirical
            .max(self.type_position.normative)
            .max(self.type_position.mythic);
...
impl DesciClaim {
    ...
    pub fn new(epistemic_tier: EpistemicTier, content: ClaimContent, creator: String) -> Self {
        Self::with_position(
            epistemic_tier,
            EpistemicPosition::default(),
            content,
            creator,
        )
    }
    ...
    /// Record legacy verification material without treating it as evidence of maturity.
    ///
    /// This is the safe ingress path for compatibility APIs while signed scientific
    /// attestations are migrated to [`crate::scientific_events`]. It rejects empty
    /// signatures and duplicate actors, but deliberately does not update MATL or E0-E4.
    pub fn record_unassessed_verification(
        &mut self,
        verification: Verification,
    ) -> crate::Result<()> {
        if verification.verifier.trim().is_empty() {
            return Err(crate::Error::Validation(
                "verification actor cannot be empty".to_string(),
            ));
        }
        if verification.signature.is_empty() {
            return Err(crate::Error::Validation(
                "verification signature material cannot be empty".to_string(),
            ));
        }
        if verification.signature.len() > 16 * 1024 {
            return Err(crate::Error::Validation(
                "verification signature material exceeds 16 KiB".to_string(),
            ));
        }
        if self
            .verifications
            .iter()
            .any(|existing| existing.verifier == verification.verifier)
        {
            return Err(crate::Error::Validation(
                "duplicate verification actor for this claim".to_string(),
            ));
        }
    ...
    /// Add a legacy verification and apply reputation-weighted tier promotion.
    ///
    /// Deprecated for untrusted API ingress. New code should use signed scientific
    /// attestations or [`Self::record_unassessed_verification`].
    ...
    pub fn add_verification_with_reputation(
        &mut self,
        verification: Verification,
        reputation: f64,
    ) {
...
mod tests {
    ...
    fn test_create_claim() {
        ...
        let claim = DesciClaim::new(EpistemicTier::E0, content, "creator_pubkey".to_string());
    ...
    #[test]
    fn test_provenance() {
        let prov = Provenance::new("PubChem ID:123".to_string(), "database".to_string())
    ...
    fn test_network_position_tracking() {
        ...
        network.add_relation(ClaimRelation::new(
            target_id,
    ...
    fn test_claim_with_fingerprint() {
        ...
        assert_eq!(
            claim.fingerprint.lem_cube.empirical,
            EmpiricalAxis::E4PubliclyReproducible
        );
    ...
    fn test_cryptographic_proof_claim() {
        ...
        assert_eq!(
            claim.fingerprint.lem_cube.normative,
            NormativeAxis::N3Axiomatic
        );
    ...
    #[test]
    fn unassessed_verification_does_not_promote_maturity() {
        let content = ClaimContent {
            dataset_hash: "hash-safe-ingress".to_string(),
            description: "Compatibility verification ingress".to_string(),
            category: "test".to_string(),
            keywords: vec![],
            storage_ref: None,
            reproducibility_score: None,
        ...
        let mut claim = DesciClaim::new(EpistemicTier::E0, content, "did:key:creator".to_string());

        claim
            .record_unassessed_verification(Verification {
                verifier: "did:key:reviewer".to_string(),
                timestamp: Utc::now(),
                signature: vec![7; 64],
                notes: None,
            })
        ...
        assert_eq!(claim.epistemic_tier, EpistemicTier::E0);
        assert_eq!(claim.fingerprint.matl_trust.raw_verifications, 0);
    }

    #[test]
    fn unassessed_verification_rejects_duplicate_actor() {
        let content = ClaimContent {
            dataset_hash: "hash-duplicate-reviewer".to_string(),
            description: "Duplicate reviewer test".to_string(),
            category: "test".to_string(),
            keywords: vec![],
            storage_ref: None,
            reproducibility_score: None,
        ...
        claim
            .record_unassessed_verification(verification.clone())
            .unwrap();
        assert!(claim.record_unassessed_verification(verification).is_err());
    }
}
