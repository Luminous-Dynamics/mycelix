...
pub struct LambertSolution {
    ...
    pub tof: f64,
    /// **NOT the maneuver delta-v for a real transfer** — this is
    /// `|v1| + |v2|`, the raw speed magnitudes of the transfer orbit's
    /// departure/arrival velocity vectors, useful only for comparing
    /// transfer orbits against EACH OTHER (e.g. picking the cheaper of two
    /// candidate TOFs) or for departures/arrivals from/to rest (deep-space
    /// endpoints). For an actual rendezvous burn cost you need
    /// `(v1 - v_departure_orbit).norm() + (v2 - v_arrival_orbit).norm()`
    /// against the ACTUAL departure/arrival orbit velocities at those
    /// points -- this crate's own `test_lambert_hohmann_agreement` computes
    /// exactly that itself rather than trusting this field. Verified
    /// 2026-07-07: zero consumers of this field exist anywhere in the
    /// monorepo outside this module, so the field is unused precisely
    /// because its name overpromises what it computes.
