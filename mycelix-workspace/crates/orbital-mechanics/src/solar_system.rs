...
/// Standard gravitational parameter (mu = G*M) of a catalog body, km^3/s^2
/// -- e.g. for use as the central body in `gravity_assist::apply_gravity_assist`
/// during a flyby of that body. Derived from the catalog's own `mass_kg`
/// via the CODATA gravitational constant in `relativity::G`, not a
/// separate hardcoded value.
pub fn mu_km3_s2(body: &SolarSystemBody) -> f64 {
    crate::relativity::G * body.mass_kg / 1e9 // m^3/s^2 -> km^3/s^2
}
...
mod tests {
    ...
    #[test]
    fn mu_km3_s2_matches_known_sun_and_earth_values() {
        // Sun: real GM_sun ~= 1.32712e11 km^3/s^2 (well-known constant).
        let sun = get_body("Sun").expect("Sun in catalog");
        let mu_sun = mu_km3_s2(&sun);
        assert!(
            (1.30e11..1.35e11).contains(&mu_sun),
            "mu_sun {mu_sun} outside expected range"
        );

        // Earth: should reproduce coordinates::wgs84::MU (398600.4418
        // km^3/s^2) to within the precision difference between the
        // catalog's rounded mass_kg and WGS-84's own defining constant --
        // a real cross-check between two independently-sourced numbers.
        let earth = get_body("Earth").expect("Earth in catalog");
        let mu_earth = mu_km3_s2(&earth);
        let wgs84_mu = crate::coordinates::wgs84::MU;
        let ratio = mu_earth / wgs84_mu;
        assert!(
            (0.99..1.01).contains(&ratio),
            "mu_earth {mu_earth} should be within 1% of wgs84::MU {wgs84_mu}"
        );
    }
