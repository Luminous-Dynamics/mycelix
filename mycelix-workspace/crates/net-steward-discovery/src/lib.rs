...
/// Stub ZK proof verification function representing the interface for v0.3-alpha.5.
/// In lab/alpha mode, this function behaves strictly and never returns `ZkProofVerdict::Verified`.
/// Every active branch must carry `proof_mode: "simulated_envelope"` and conform to `ZkVerifierPolicy`.
pub fn verify_claim_zk_stub(request: &ZkProofRequest, policy: ZkVerifierPolicy) -> ZkProofVerdict {
    ...
    match policy {
        ZkVerifierPolicy::Disabled => ZkProofVerdict::Disabled {
            request_id: request.request_id.clone(),
            proof_mode: "simulated_envelope".to_string(),
        },
        ZkVerifierPolicy::SimulatedOnly => ZkProofVerdict::SimulatedEnvelope {
            request_id: request.request_id.clone(),
            verified_at_unix_ms: now,
            proof_mode: "simulated_envelope".to_string(),
        },
        ZkVerifierPolicy::GatedBehindFeatureFlag => ZkProofVerdict::Rejected {
            request_id: request.request_id.clone(),
            reason: "ZK verification gated behind feature flag".to_string(),
            proof_mode: "simulated_envelope".to_string(),
        },
    }
}
