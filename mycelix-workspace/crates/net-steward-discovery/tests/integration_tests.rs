...
// ─── v0.3-alpha.5: ZK Verifier Interface Tests ────────────────────────────

/// Happy path stub: verify_claim_zk_stub with SimulatedOnly must return
/// ZkProofVerdict::SimulatedEnvelope, have proof_mode == "simulated_envelope",
/// and must NEVER return ZkProofVerdict::Verified.
#[test]
fn test_zk_verifier_stub_always_returns_simulated_in_lab_mode() {
    let request = ZkProofRequest {
        request_id: "zk-req-123".to_string(),
        claim_hash: "sha256-posture-hash-abc123xyz".to_string(),
        proof_payload: vec![1, 2, 3, 4, 5],
        public_inputs: vec!["input-commitment-0".to_string()],
        verification_key_id: "vkey-v1.0".to_string(),
    };

    let verdict = verify_claim_zk_stub(&request, ZkVerifierPolicy::SimulatedOnly);

    // Assert that ZkProofVerdict::Verified is never returned
    assert!(
        !matches!(verdict, ZkProofVerdict::Verified { .. }),
        "ZK verifier stub must never return a Verified verdict in lab/alpha mode"
    );

    // Assert it returns SimulatedEnvelope
    match verdict {
        ZkProofVerdict::SimulatedEnvelope {
            request_id,
            verified_at_unix_ms,
            proof_mode,
        } => {
            assert_eq!(request_id, "zk-req-123");
            assert!(verified_at_unix_ms > 0);
            assert_eq!(proof_mode, "simulated_envelope");
        }
        other => panic!(
            "Expected ZkProofVerdict::SimulatedEnvelope, got {:?}",
            other
        ),
    }
}

/// Feature gating and disabled policies: verify_claim_zk_stub with
/// Disabled or GatedBehindFeatureFlag must return Disabled or Rejected,
/// and must NEVER return ZkProofVerdict::Verified.
#[test]
fn test_zk_verifier_gated_behind_feature_flag_or_disabled() {
    let request = ZkProofRequest {
        request_id: "zk-req-456".to_string(),
        claim_hash: "sha256-posture-hash-def456uvw".to_string(),
        proof_payload: vec![9, 8, 7],
        public_inputs: vec!["input-commitment-1".to_string()],
        verification_key_id: "vkey-v1.0".to_string(),
    };

    // 1. Test GatedBehindFeatureFlag policy
    let verdict_gated = verify_claim_zk_stub(&request, ZkVerifierPolicy::GatedBehindFeatureFlag);
    assert!(
        !matches!(verdict_gated, ZkProofVerdict::Verified { .. }),
        "ZK verifier stub must never return a Verified verdict when gated"
    );

    match verdict_gated {
        ZkProofVerdict::Rejected {
            request_id,
            reason,
            proof_mode,
        } => {
            assert_eq!(request_id, "zk-req-456");
            assert!(reason.contains("gated behind feature flag"));
            assert_eq!(proof_mode, "simulated_envelope");
        }
        other => panic!("Expected ZkProofVerdict::Rejected, got {:?}", other),
    }

    // 2. Test Disabled policy
    let verdict_disabled = verify_claim_zk_stub(&request, ZkVerifierPolicy::Disabled);
    assert!(
        !matches!(verdict_disabled, ZkProofVerdict::Verified { .. }),
        "ZK verifier stub must never return a Verified verdict when disabled"
    );

    match verdict_disabled {
        ZkProofVerdict::Disabled {
            request_id,
            proof_mode,
        } => {
            assert_eq!(request_id, "zk-req-456");
