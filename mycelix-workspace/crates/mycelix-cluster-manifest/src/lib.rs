...
/// Cryptographic assurance level for a cluster.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
...
mod tests {
    ...
    fn sample_cluster(
        id: &str,
        deps: Vec<ClusterDependency>,
        bridges: Vec<BridgeDeclaration>,
    ) -> ClusterManifest {
        ClusterManifest {
            id: id.to_string(),
            name: id.to_string(),
    ...
    fn catalog_find_cluster() {
        ...
        catalog
            .clusters
            .push(sample_cluster("health", vec![], vec![]));
        catalog
            .clusters
            .push(sample_cluster("governance", vec![], vec![]));
    ...
    fn data_flows_between_installed() {
        let mut catalog = ClusterCatalog::new();
        catalog.clusters.push(sample_cluster(
            "health",
            vec![],
            vec![BridgeDeclaration {
        ...
        catalog
            .clusters
            .push(sample_cluster("identity", vec![], vec![]));
