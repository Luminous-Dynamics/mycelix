...
use crate::curriculum::{CurriculumNode, ProgressStatus, curriculum_graph, use_progress};
use leptos::prelude::*;
        ...
        let mut certs: Vec<_> = graph
            ...
            .filter(|n| {
                search.is_empty()
                    ...
                    || n.domain.to_lowercase().contains(&search)
            })
            .filter(|n| {
                if sector_filter == MetabolicSector::All {
                    return true;
                }
                match sector_filter {
                    MetabolicSector::DigitalMesh => {
            ...
            .map(|n| {
                ...
                let missing: Vec<(String, bool)> = prereqs
                    .iter()
                    ...
                    .map(|pid| {
                        let is_ready = graph
                            .prereqs_for(pid)
                            .iter()
                            .all(|ppid| mastered_set.contains(*ppid));
                        let title = graph
                            .node(pid)
