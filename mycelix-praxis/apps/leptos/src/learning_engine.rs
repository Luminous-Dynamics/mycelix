...
use crate::consciousness::{ConsciousnessState, use_consciousness};
...
pub fn compute_recommendation(
    ...
    let reason = match &readiness {
        LearningReadiness::FlowState => "You're in the zone! Let's go deeper.".to_string(),
        LearningReadiness::Ready => "Good focus. Let's learn something new.".to_string(),
        LearningReadiness::NeedsWarmup => {
        ...
        LearningReadiness::Disengaged => "Let's try something different to re-engage.".to_string(),
    };

    // Override action if break interval exceeded (unless already a break)
    let final_action = if needs_break && !matches!(action, RecommendedAction::TakeBreak { .. }) {
...
mod tests {
    ...
    fn test_flow_state_high_mastery_recommends_creative_project() {
        ...
        assert!(
            rec.difficulty_adjustment > 0.0,
            "Flow state should push harder"
        );
    }
    ...
    fn test_kindergarten_break_duration_is_10_min() {
        let rec = compute_recommendation(
            0.1, 0.5, 0.8, // cortisol: HIGH
            0.3, 0.5, 500, 1, // Kindergarten (grade_ordinal = 1)
            5, None,
        );
        assert_eq!(
            rec.recommended_action,
    ...
    fn test_grade5_break_duration_is_5_min() {
        let rec = compute_recommendation(
            0.1, 0.5, 0.8, // cortisol: HIGH
            0.3, 0.5, 500, 6, // Grade 5 (grade_ordinal = 6)
            5, None,
        );
        assert_eq!(
            rec.recommended_action,
    ...
    fn test_needs_warmup_state() {
        ...
        assert!(
            rec.difficulty_adjustment < 0.0,
            "Warmup should reduce difficulty"
        );
    }
    ...
    fn test_hint_shown_at_low_consciousness() {
        let rec = compute_recommendation(
            ...
            0.2, 0.3, 0.3, 500, 5, 10, None,
        );
        assert!(
            rec.should_show_hint,
            "Should show hint when consciousness < 0.35"
        );
    }
    ...
    fn test_hint_shown_at_low_dopamine() {
        let rec = compute_recommendation(
            ...
            0.2, 0.5, 0.3, 500, 5, 10, None,
        );
        assert!(
            rec.should_show_hint,
            "Should show hint when dopamine < 0.25"
        );
    }
    ...
    fn test_high_acetylcholine_defaults_to_readwrite() {
        let rec = compute_recommendation(
            0.5, 0.5, 0.2, 0.7, // acetylcholine > 0.6
            0.3, // oxytocin <= 0.5
            500, 5, 10, None,
        );
    ...
    fn test_high_oxytocin_defaults_to_kinesthetic() {
        let rec = compute_recommendation(
            0.5, 0.5, 0.2, 0.4, // acetylcholine <= 0.6
            0.6, // oxytocin > 0.5
            500, 5, 10, None,
        );
    ...
    fn test_session_time_remaining_calculation() {
        ...
        let rec = compute_recommendation(0.5, 0.5, 0.2, 0.5, 0.5, 500, 5, 20, None);
    ...
    fn test_session_time_remaining_does_not_underflow() {
        ...
        let rec = compute_recommendation(0.5, 0.5, 0.2, 0.5, 0.5, 500, 0, 100, None);
    ...
    fn test_neuromod_proxy_cortisol_high_stress() {
        ...
        assert!(
            proxies.cortisol > 0.65,
            "High NE + low 5HT should indicate stress"
        );
    }
    ...
    fn test_neuromod_proxy_cortisol_low_stress() {
        ...
        assert!(
            proxies.cortisol < 0.1,
            "Low NE + high 5HT should indicate calm"
        );
    }
    ...
    fn test_grade12_mastery_threshold() {
        ...
        let rec = compute_recommendation(0.7, 0.6, 0.2, 0.7, 0.5, 800, 13, 10, None);
        assert_eq!(rec.readiness, LearningReadiness::FlowState);
        assert_eq!(rec.recommended_action, RecommendedAction::CreativeProject);
    }
}
