...
impl InteractionMode {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            Self::Guardian => {
                "I'll help guide you and explain what I'm doing. You asked for extra support — you can stop anytime."
            }
...
pub fn curiosity_prompt(recent_topics: &[String], grade_ordinal: u8) -> CuriosityPrompt {
...
pub fn adapt_content(
            ...
            suggestion: Some(Suggestion {
                ...
                options: vec![
                    StudentChoice {
                        ...
                        action: ChoiceAction::TakeBreak {
                            minutes: break_mins,
                        },
                        is_decline: false,
                    },
                    StudentChoice {
                        label: "I'm really OK, let me finish this problem first".into(),
    ...
    let frustration_score =
        (-state.valence).max(0.0) * state.arousal * (consecutive_failures as f32 / 3.0).min(1.0);
    ...
    let text_complexity =
        if !is_productive_struggle && consecutive_failures >= 2 && recent_accuracy < 0.4 {
    ...
    {
        Some(PeerSuggestion {
            reason: format!("{} might be easier with a study buddy", current_skill),
            ...
            duration_minutes: 10,
            expected_benefit:
                "Students who explain concepts to peers deepen their own understanding".into(),
        })
    } else {
        None
    };
...
fn build_suggestion(
    ...
    if state.motor_command == MotorCommand::ReflectionInitiate {
        return Some(Suggestion {
            ...
            message: "Nice work! Can you explain how you solved that in your own words?".into(),
            options: vec![
                StudentChoice {
...
pub fn metacognitive_prompt(
    ...
    match sovereignty.mode() {
        ...
        InteractionMode::Guide => Some(match (recent_result, consecutive_correct) {
            (true, n) if n >= 5 => {
                "You're on fire! Is this getting too easy? You can make it harder.".into()
            }
            (true, _) => "Nice! How did you figure that out?".into(),
            (false, _) => "Hmm, that didn't work. What would you try differently?".into(),
        }),
...
pub fn suggest_rewrite(
    ...
    match sovereignty.mode() {
        InteractionMode::Guardian => RewriteResult::Applied {
            ...
            explanation: "I made the words simpler to help you focus on the math.".into(),
        },
        ...
        InteractionMode::Mirror | InteractionMode::Autonomous => RewriteResult::Available {
            original: original.to_string(),
        },
    }
}
...
pub fn rewrite_problem(original: &str, complexity: &TextComplexity, grade_ordinal: u8) -> String {
    match complexity {
...
pub fn extract_equation(text: &str) -> String {
    if text.contains('\u{00d7}') || text.contains('*') || text.contains('+') || text.contains('-') {
        text.chars()
            .filter(|c| {
                c.is_ascii_digit()
                    || *c == '\u{00d7}'
...
mod tests {
    ...
    fn metacognitive_guardian_streak() {
        let prompt = metacognitive_prompt(&guardian(), true, 3, "addition");
    ...
    fn derive_cognitive_state_cortisol() {
        let state = derive_cognitive_state(0.5, 0.5, 0.5, 0.2, 0.8, MotorCommand::NoOp);
        assert!((state.stress - 0.64).abs() < 0.01);
    }

    #[test]
    fn derive_cognitive_state_valence() {
        let state = derive_cognitive_state(0.5, 0.5, 0.3, 0.2, 0.8, MotorCommand::NoOp);
    ...
    fn derive_cognitive_state_permeability_sigmoid() {
        ...
        let closed = derive_cognitive_state(0.5, 0.5, 0.3, 0.1, 0.9, MotorCommand::NoOp);
        assert!(closed.permeability < 0.4);
    }

    #[test]
    fn derive_cognitive_state_oxytocin() {
        let state = derive_cognitive_state(0.5, 0.5, 0.6, 0.8, 0.3, MotorCommand::NoOp);
    ...
    fn high_phi_high_mastery_offers_challenge() {
        ...
        assert_eq!(
            suggestion.suggestion_type,
            SuggestionType::CelebrateProgress
        );
        assert!(suggestion.options.iter().any(|o| o.is_decline));
    }
    ...
    fn serialization_roundtrip() {
        ...
        let _: ContentAdaptation = serde_json::from_str(&json).expect("deserialize");
    }

    #[test]
    fn sovereignty_serialization_roundtrip() {
        let mut sov = SovereigntyLevel::new();
        sov.record_growth(SovereigntyGrowthType::AskedForHelp, "test".into(), 42);
        let json = serde_json::to_string(&sov).expect("serialize");
        let deser: SovereigntyLevel = serde_json::from_str(&json).expect("deserialize");
