        ...
        if let Some(doc) = web_sys::window().and_then(|w| w.document()) {
            if let Some(el) = doc.document_element() {
                if let Some(style) = el.dyn_ref::<web_sys::HtmlElement>().map(|e| e.style()) {
                    let _ = style
                        .set_property("--consciousness-warmth", &format!("{:.3}", self.warmth));
                    let _ = style
                        .set_property("--consciousness-density", &format!("{:.3}", self.density));
                    let _ = style.set_property(
                        "--consciousness-chrome-opacity",
                        &format!("{:.3}", self.chrome_opacity),
                    );
                    let _ = style.set_property(
                        "--consciousness-animation-speed",
                        &format!("{:.3}", self.animation_speed),
                    );
                    let _ = style.set_property(
                        "--consciousness-accent-saturation",
                        &format!("{:.3}", self.accent_saturation),
                    );
                }
            }
        }
