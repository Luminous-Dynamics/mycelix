...
fn canonical_graph_bytes(graph: &SettlementGraphV1) -> Result<Vec<u8>, GraphError> {
    ...
    push_count(&mut out, graph.legs.len())?;
    for leg in &graph.legs {
        push_digest(&mut out, leg.leg_id);
    }

    push_count(&mut out, graph.dependencies.len())?;
    for dependency in &graph.dependencies {
        push_dependency(&mut out, dependency)?;
    }

    push_count(&mut out, graph.coordination_groups.len())?;
    for group in &graph.coordination_groups {
        push_digest(&mut out, group.group_id);
    }
...
fn push_dependency(
    ...
    match dependency {
        SettlementDependencyV1::Requires {
            ...
            push_u8(out, 1);
            push_digest(out, *leg_id);
            push_digest(out, *prerequisite_id);
        }
        SettlementDependencyV1::Before {
            ...
            push_u8(out, 2);
            push_digest(out, *before_id);
            push_digest(out, *after_id);
        }
        SettlementDependencyV1::ConditionalOnEvidence {
            ...
            push_u8(out, 3);
            push_digest(out, *leg_id);
            push_profile(out, predicate_profile)?;
        }
    }
    Ok(())
}
