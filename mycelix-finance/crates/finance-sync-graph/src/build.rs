...
use crate::canonical::{
    derive_graph_commitment, derive_group_commitment, derive_leg_commitment,
};
use crate::{
    ...
    SettlementGraphV1, SettlementLegV1, MAX_DEPENDENCIES, MAX_GROUPS,
    MAX_GROUP_MEMBERS, MAX_LEGS, MIN_LEGS,
};
...
pub fn build_settlement_graph_v1(
    ...
    for spec in leg_specs {
        ...
        let leg_id = derive_leg_commitment(
            economic_effect_commitment,
            &graph_profile,
            &spec,
        )?;
    ...
    let coordination_groups =
        resolve_groups(group_specs, &aliases, &graph_profile)?;
