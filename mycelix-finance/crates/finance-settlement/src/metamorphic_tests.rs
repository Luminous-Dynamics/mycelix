...
fn observation(
    ...
    SettlementObservation {
        ...
        state,
        observed_at_unix_ms,
        evidence: vec![evidence(subject, &observation_id, operation, revision, suffix)],
    }
}

fn qualified_three_operation_corpus(
    subject: &SettlementSubject,
) -> Vec<SettlementObservation> {
    vec![
        observation(
            subject,
            "op:a",
...
fn higher_current_revision_never_falls_back_to_lower_applied_revision() {
    ...
    for state in [ObservedSettlementState::Rejected, ObservedSettlementState::Reversed] {
...
fn context_class_changes_proof_identity_even_at_the_same_timestamp() {
    ...
    let historical_result = qualify_settlement(
        &subject,
        &profile,
        &[observation],
        &historical,
...
fn push_reference(out: &mut Vec<u8>, value: &str) {
    push_u32(out, u32::try_from(value.len()).expect("bounded test reference"));
