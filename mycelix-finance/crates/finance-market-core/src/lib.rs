...
/// Canonical FIN-MKT protocol vocabulary identifier.
///
/// Protocol/profile names are deliberately stricter than domain-owned subject
/// identifiers. V1 accepts only visible ASCII alphanumerics plus `._:/-` and
/// performs no case folding or normalization.
...
const fn is_protocol_id_byte(value: u8) -> bool {
    value.is_ascii_alphanumeric()
        || matches!(value, b'.' | b'_' | b':' | b'/' | b'-')
}

/// Bounded exact UTF-8 identifier owned/interpreted by a domain or adapter.
///
/// No Unicode, URI, DID, provider-symbol or scheme-specific normalization is
/// performed. Different exact UTF-8 byte spellings remain different IDs unless
/// an explicit mapping profile proves a relation.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize)]
#[serde(transparent)]
...
impl CanonicalMarketOrderIntentV1 {
    ...
    /// Return the exact canonical transcript for an already-validated intent.
