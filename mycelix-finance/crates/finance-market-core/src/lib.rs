...
pub const MAX_MARKET_ID_BYTES: usize = 256;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            Self::InvalidIdentifier => {
                "identifier is empty, too long, or contains a control character"
            }
...
/// Bounded exact UTF-8 market-domain identifier.
///
/// No Unicode normalization or scheme-specific equivalence is performed. Two
/// different byte spellings remain different identifiers. Provider adapters own
/// any stronger identifier interpretation.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize)]
#[serde(transparent)]
pub struct MarketIdV1(String);

impl MarketIdV1 {
    pub fn new(value: impl Into<String>) -> Result<Self, MarketIntentError> {
        let value = value.into();
        if value.is_empty()
            || value.len() > MAX_MARKET_ID_BYTES
            || value.chars().any(char::is_control)
        {
            return Err(MarketIntentError::InvalidIdentifier);
        }
        Ok(Self(value))
    }
...
impl fmt::Display for MarketIdV1 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(&self.0)
    }
}

impl<'de> Deserialize<'de> for MarketIdV1 {
...
/// Exact opaque 32-byte commitment with one canonical lowercase-hex JSON form.
...
pub struct MarketPriceV1 {
    /// Exact quote amount under `pricing_profile`.
    ///
    /// The profile, not this generic type, defines the base lot / scaling rule.
...
/// Caller-supplied semantic candidate. Successful validation establishes only a
/// deterministic immutable order-intent identity.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
/// Constructor-controlled canonical order intent.
///
/// This is deliberately not `Deserialize`: portable bytes are reconstruction
/// inputs, not authority to manufacture a positive semantic object.
...
impl CanonicalMarketOrderIntentV1 {
    ...
    pub fn canonical_bytes(&self) -> Vec<u8> {
        canonical_order_intent_bytes_v1(&self.input)
    }
}
...
pub fn canonicalize_order_intent_v1(
    ...
    let bytes = canonical_order_intent_bytes_v1(&input);
    let intent_commitment = sha256_digest(&bytes);
...
pub fn canonical_order_intent_bytes_v1(input: &MarketOrderIntentInputV1) -> Vec<u8> {
...
fn push_text(output: &mut Vec<u8>, value: &str) {
    let len = u32::try_from(value.len()).expect("bounded market identifiers fit u32");
...
mod tests {
    use super::*;
    use mycelix_finance_exact::{AssetId, AssetAmount};
    ...
    fn profile(id: &str, byte: u8) -> MarketProfileRefV1 {
        MarketProfileRefV1 {
            profile_id: MarketIdV1::new(id).unwrap(),
            revision: 1,
    ...
    fn fixture() -> MarketOrderIntentInputV1 {
        MarketOrderIntentInputV1 {
            ...
            semantic_idempotency: MarketIdempotencyRefV1 {
                ...
                semantic_id: MarketIdV1::new("idem:demo:001").unwrap(),
            },
    ...
    fn json_fixture_reconstructs_frozen_vector() {
        let input: MarketOrderIntentInputV1 =
    ...
    fn effect_significant_mutations_change_identity() {
        ...
        let mut changed = fixture();
        changed.semantic_idempotency.semantic_id = MarketIdV1::new("idem:demo:002").unwrap();
        assert_ne!(
            base.commitment(),
            canonicalize_order_intent_v1(changed).unwrap().commitment()
        );
    ...
    fn digest_wire_rejects_noncanonical_uppercase() {
        ...
        assert_eq!(
            Digest32::from_lower_hex(&uppercase),
            Err(MarketIntentError::InvalidDigest)
        );
        let lowercase = "aa".repeat(32);
        assert!(Digest32::from_lower_hex(&lowercase).is_ok());
    }
