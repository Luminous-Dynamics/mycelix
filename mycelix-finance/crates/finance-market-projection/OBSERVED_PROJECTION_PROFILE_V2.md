...
`ProjectionScopeV2` binds the exact event-observation profile and the exact FIN-MKT-002B0 execution-frontier scope. The supplied execution frontier must bind the same immutable intent commitment and exact execution scope.
...
```text
Resolved + zero known execution identities
-> NoExecutionObserved

Resolved + historical execution identities + zero effective executions
-> HistoricalExecutionOnlyObserved

Resolved + effective executions + no adjustments
-> EffectiveExecutionObserved

Resolved + effective executions + adjustments
...
This preserves the critical theorem:

```text
busted/corrected-away execution
!= execution never observed
```

Historical observations are not erased merely because adjustment resolution removes an execution from the effective set.
