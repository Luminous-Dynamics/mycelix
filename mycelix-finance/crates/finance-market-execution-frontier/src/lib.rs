    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            ...
            Self::ExecutionIdentityConflict => {
                "one provider execution identity is reused for different economic execution semantics"
            }
...
impl ExecutionRecord {
    fn economics_match(&self, fill: &CanonicalFillObservationV1) -> bool {
        self.executed_quantity == fill.input().executed_quantity
            && self.execution_price == fill.input().execution_price
            && self.venue_ref == fill.input().venue_ref
    }
...
fn deduplicate_fill_observations<'a>(
    fills: &'a [CanonicalFillObservationV1],
) -> Result<Vec<&'a CanonicalFillObservationV1>, ExecutionFrontierError> {
...
fn deduplicate_adjustment_observations<'a>(
    adjustments: &'a [CanonicalFillAdjustmentObservationV1],
) -> Result<Vec<&'a CanonicalFillAdjustmentObservationV1>, ExecutionFrontierError> {
...
fn resolve_execution_identities<'a>(
    fills: &[&'a CanonicalFillObservationV1],
) -> Result<
    (
...
> {
    ...
    for fill in fills {
        ...
        match executions.get_mut(&execution_ref) {
            ...
            Some(existing) if existing.economics_match(fill) => {
                existing
                    .fill_semantic_aliases
                    .insert(fill.semantic_commitment());
            }
            Some(_) => return Err(ExecutionFrontierError::ExecutionIdentityConflict),
        }
...
mod tests {
    ...
    use mycelix_finance_market_core::{
        canonicalize_order_intent_v1, Digest32, MarketIdempotencyRefV1, MarketInstrumentRefV1,
    ...
    use mycelix_finance_market_observation::{
        ...
        FillAdjustmentObservationInputV1, FillObservationInputV1, MarketOrderObservationSubjectV1,
    ...
    fn fill(
        ...
        canonicalize_fill_observation_v1(FillObservationInputV1 {
            ...
            venue_ref: Some(MarketExternalIdV1::new("venue:demo").unwrap()),
    ...
    fn same_execution_across_two_observation_ids_counts_once() {
        let intent = intent();
        let first = fill(&intent, "fill-envelope:1", "exec:1", 40, 18_750, 10, 0x91);
        let second = fill(&intent, "fill-envelope:2", "exec:1", 40, 18_750, 11, 0x92);
    ...
    fn same_execution_ref_with_changed_quantity_conflicts() {
        let intent = intent();
        let first = fill(&intent, "fill-envelope:1", "exec:1", 40, 18_750, 10, 0x91);
        let changed = fill(&intent, "fill-envelope:2", "exec:1", 41, 18_750, 11, 0x92);
    ...
    fn identical_economics_with_different_execution_refs_are_distinct() {
        let intent = intent();
        let first = fill(&intent, "fill-envelope:1", "exec:1", 40, 18_750, 10, 0x91);
        let second = fill(&intent, "fill-envelope:2", "exec:2", 40, 18_750, 11, 0x92);
    ...
    fn correction_can_reference_one_alias_of_duplicated_execution() {
        let intent = intent();
        let prior_first = fill(&intent, "fill-envelope:1", "exec:1", 40, 18_750, 10, 0x91);
        let prior_alias = fill(&intent, "fill-envelope:2", "exec:1", 40, 18_750, 11, 0x92);
        let replacement = fill(&intent, "fill-envelope:3", "exec:2", 45, 18_700, 12, 0x93);
    ...
    fn correction_between_two_aliases_of_same_execution_conflicts() {
        let intent = intent();
        let first = fill(&intent, "fill-envelope:1", "exec:1", 40, 18_750, 10, 0x91);
        let alias = fill(&intent, "fill-envelope:2", "exec:1", 40, 18_750, 11, 0x92);
        let correction = adjustment(
            &intent,
            "adjust:1",
            FillAdjustmentKindV1::Correction {
