...
Two observations with one execution ref describe the same economic execution only when all V1 economic fields are exactly equal:
...
```text
same execution ref + same exact economics + different observation IDs
-> one observed economic execution

same execution ref + different exact economics
...
5. correction referencing one alias of a duplicated execution -> one corrected execution;
6. correction prior/replacement collapse to same execution -> conflict;
7. missing adjustment ref -> indeterminate;
8. bust removes execution only from effective frontier;
9. correction chain resolves to terminal replacement;
10. cycle -> conflict;
11. provider-order-ref conflict -> fail closed;
12. arbitrary bounded frontier -> typed result/error, never panic.
