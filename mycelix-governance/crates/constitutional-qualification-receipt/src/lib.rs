...
/// this value is not proof that a CI authority actually issued the receipt.
...
impl QualificationReceiptV1 {
    pub fn new(
        ...
        receipt.validate_authority_tuple()?;
        receipt.navigation.validate()?;
        receipt.receipt_commitment = receipt.compute_authority_commitment();
        Ok(receipt)
    }

    pub fn validate(&self) -> ReceiptResult<()> {
        self.validate_authority_tuple()?;
    ...
    fn validate_authority_tuple(&self) -> ReceiptResult<()> {
        if self.schema_version != RECEIPT_SCHEMA_VERSION {
            return Err(invalid(
                "schema_version",
                format!("must equal {RECEIPT_SCHEMA_VERSION}"),
            ));
        }
        require_opaque(
            "dependency_id",
...
/// Exact qualification tuple required by an activation consumer.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl QualificationRequirementV1 {
    ...
    pub fn validate_receipt(&self, receipt: &QualificationReceiptV1) -> ReceiptResult<()> {
        ...
        self.validate_identity(&receipt.semantic_head, &receipt.verifier_head)
    }

    fn validate_identity(&self, semantic_head: &str, verifier_head: &str) -> ReceiptResult<()> {
...
///
/// Missing, extra, duplicate, pending, failed, wrong-semantic, or wrong-verifier
    ...
    for item in evidence {
        ...
        match item {
            ...
            QualificationEvidenceV1::Pending(pending) => {
                requirement.validate_identity(&pending.semantic_head, &pending.verifier_head)?;
                return Err(ReceiptError::DependencyPending(dependency_id.to_string()));
            }
            QualificationEvidenceV1::Failed(failed) => {
                requirement.validate_identity(&failed.semantic_head, &failed.verifier_head)?;
...
mod tests {
    ...
    #[test]
    fn zero_run_id_and_malformed_digest_fail_closed() {
        assert!(QualificationReceiptV1::new(
            "MYC-CONST-TEST-A".into(),
            git_hex('a'),
            git_hex('b'),
            0,
            sha256_hex('c'),
            ReceiptNavigationV1::default(),
        )
        .is_err());
    ...
    #[test]
    fn requirement_rejects_wrong_semantic_and_verifier_heads() {
    ...
    fn exact_census_rejects_duplicate_requirements() {
        ...
        assert!(matches!(
            validate_exact_qualified_census(
