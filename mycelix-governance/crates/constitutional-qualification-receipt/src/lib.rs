...
/// Exact qualification lineage required by an evidence consumer.
///
...
/// Validate an exact census of *presented* evidence.
///
/// This is a structural/deterministic check only. It rejects missing, extra,
/// duplicate, pending, failed, wrong-issuer, wrong-semantic, and wrong-verifier
/// evidence, but ordinary `QualificationReceiptV1` values remain caller-
/// constructible. Do not use this function as the final activation authority.
pub fn validate_exact_presented_census(
...
/// Validate an exact census using only receipts that already crossed the
/// separately-qualified authenticity boundary.
///
/// Production callers cannot construct `VerifiedQualificationReceiptV1` in
/// QREC-001. This keeps activation fail-closed until an ingestion successor earns
/// a constructor.
pub fn validate_exact_verified_census(
    requirements: &[QualificationRequirementV1],
    receipts: &[VerifiedQualificationReceiptV1],
) -> ReceiptResult<()> {
    let mut requirement_ids = BTreeSet::new();
    for requirement in requirements {
        requirement.validate()?;
    ...
    for verified in receipts {
        ...
        receipt.validate()?;
        if !receipt_ids.insert(receipt.dependency_id.as_str()) {
            return Err(ReceiptError::DuplicateEvidence(
                receipt.dependency_id.clone(),
            ));
        }
        let Some(requirement) = requirements
            .iter()
            .find(|requirement| requirement.dependency_id == receipt.dependency_id)
        else {
            return Err(ReceiptError::UnexpectedEvidence(
                receipt.dependency_id.clone(),
            ));
    ...
    for requirement in requirements {
        if !receipt_ids.contains(requirement.dependency_id.as_str()) {
            return Err(ReceiptError::MissingEvidence(
                requirement.dependency_id.clone(),
            ));
        }
...
#[cfg(test)]
impl VerifiedQualificationReceiptV1 {
    fn test_only_from_receipt(receipt: QualificationReceiptV1) -> Self {
        Self {
            receipt,
            _authenticated_ingestion: VerifiedIngestionSeal,
        }
...
mod tests {
    ...
    #[test]
    fn presented_census_accepts_only_structurally_exact_qualified_receipts() {
        ...
        validate_exact_presented_census(&requirements, &evidence).unwrap();
    }

    #[test]
    fn presented_census_rejects_duplicate_missing_and_extra_evidence() {
        let requirements = vec![
            requirement("MYC-CONST-TEST-A", 'a', 'b'),
            requirement("MYC-CONST-TEST-B", 'c', 'd'),
        ];
        ...
        assert!(matches!(
            validate_exact_presented_census(&requirements, &[a.clone(), a.clone()]),
            Err(ReceiptError::DuplicateEvidence(_))
        ));
        assert!(matches!(
            validate_exact_presented_census(&requirements, std::slice::from_ref(&a)),
        ...
        assert!(matches!(
            validate_exact_presented_census(&requirements[..1], &[a, extra]),
            Err(ReceiptError::UnexpectedEvidence(_))
        ));
    }

    #[test]
    fn presented_census_rejects_pending_and_failed_dependencies() {
        ...
        assert_eq!(
            validate_exact_presented_census(&requirements, &[pending]),
        ...
        assert!(matches!(
            validate_exact_presented_census(&requirements, &[failed]),
            Err(ReceiptError::DependencyFailed { .. })
        ));
    }

    #[test]
    fn presented_census_rejects_duplicate_requirements() {
        let requirement = requirement("MYC-CONST-TEST-A", 'a', 'b');
        ...
        assert!(matches!(
            validate_exact_presented_census(&[requirement.clone(), requirement], &[evidence]),
            Err(ReceiptError::DuplicateRequirement(_))
        ));
    }

    #[test]
    fn verified_census_requires_verified_wrapper_type() {
        let expected = receipt("MYC-CONST-TEST-A", 'a', 'b', 11, 'e');
        let requirements = vec![requirement("MYC-CONST-TEST-A", 'a', 'b')];
        let verified = VerifiedQualificationReceiptV1::test_only_from_receipt(expected);
        validate_exact_verified_census(&requirements, &[verified]).unwrap();
    }
}
