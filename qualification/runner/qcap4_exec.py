...
LAUNCH_HANDSHAKE_CEILING_SECONDS = 1.0
...
def _kill_direct_bounded(process, ceiling_seconds=0.1):
    if process.poll() is None:
        try:
            ...
            pass
    return _wait_direct(process, ceiling_seconds)


def _abort_containment(process, cgroup, cleanup_ceiling_ms):
    try:
        terminate_and_cleanup(cgroup, cleanup_ceiling_ms)
        direct_stopped = _wait_direct(process, cleanup_ceiling_ms / 1000)
        reap_children_bounded(cleanup_ceiling_ms)
        if not direct_stopped:
            _kill_direct_bounded(process, 0.05)
            return False
        return True
    except ContainmentError:
        _kill_direct_bounded(process, 0.1)
        reap_children_bounded(min(cleanup_ceiling_ms, 100))
        return False
...
def _finish_containment(process, cgroup, cleanup_ceiling_ms):
    try:
        ...
        if not _wait_direct(process, 0.05):
            _kill_direct_bounded(process, 0.05)
            return False
        reap_children_bounded(cleanup_ceiling_ms)
        return True
